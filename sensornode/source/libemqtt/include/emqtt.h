...
#define MQTT_USERNAME_LENGTH 32 // Recommended by MQTT Specification (12 + '\0')
#define MQTT_PASSWORD_LENGTH 64 // Recommended by MQTT Specification (12 + '\0')
#define MQTT_CLIENTID_LENGTH_MAX 50
...
#define MQTT_CLIENT_ID_LEN_MAX 50
...
typedef struct _mqtt_broker_handle  mqtt_broker_handle_t;
typedef struct _mqtt_net_if         mqtt_net_if_t;

struct _mqtt_broker_handle{
	void* socket_info;
    // Socket functions
    mqtt_net_if_t const *net_if;
	// Management fields
	uint16_t seq;
	uint16_t alive;
    // tx/rx buf
    char *buf;
	// Connection info
	char clientid[MQTT_CLIENTID_LENGTH_MAX];
	// Auth fields
...
struct _mqtt_net_if {
    int (*init)(mqtt_broker_handle_t* broker, const char* hostname, uint16_t port);
    int (*close)(mqtt_broker_handle_t* broker);
    int (*send)(void* socket_info, const void* buf, unsigned int len);
    int (*recv)(void* socket_info, char* buf, unsigned int len, int timeout);
};
 ...
 * @param broker Data structure that contains the connection information with the broker.
 * @param host MQTT broker host name or IP address string
