...
# define MQTT_RECV_BUF_SIZE 128
#endif

#ifndef  MQTT_RECV_TIMEOUT
# define MQTT_RECV_TIMEOUT 3000
#endif

static char RecvBuf[MQTT_RECV_BUF_SIZE];
...
void mqtt_init(mqtt_broker_handle_t* broker, const char* clientid, mqtt_net_if_t const *net_if) {
	// Connection options
...
void mqtt_init_auth(mqtt_broker_handle_t* broker, const char* username, const char* password) {
	if(username && username[0] != '\0') strncpy(broker->username, username, sizeof(broker->username)-1);
	if(password && password[0] != '\0') strncpy(broker->password, password, sizeof(broker->password)-1);
}
...
int mqtt_connect(mqtt_broker_handle_t* broker, char const *host, uint16_t port)
{
	uint8_t flags = 0x00;

	size_t clientidlen = strlen(broker->clientid);
	size_t usernamelen = strlen(broker->username);
	size_t passwordlen = strlen(broker->password);
    ...
    uint8_t *fixed_header = malloc(fixedHeaderSize * sizeof(uint8_t));
	uint8_t *packet       = malloc(fixedHeaderSize + sizeof(var_header) + payload_len);
    if (!fixed_header || !packet) {
        if (fixed_header) free(fixed_header);
    ...
    if (broker->net_if->init(broker, host, port) >= 0) {
        // Send the connect message
        if(broker->net_if->send(broker->socket_info, packet, offset) < 0) {
            broker->net_if->close(broker);
            if (fixed_header) free(fixed_header);
            if (packet) free(packet);
            ret = -1;
        }
    ...
    // Wait for CONNACK
    if (broker->net_if->recv(broker->socket_info, RecvBuf, MQTT_RECV_BUF_SIZE, MQTT_RECV_TIMEOUT) >= 0) {
        if (MQTTParseMessageType(RecvBuf) != MQTT_MSG_CONNACK) {
            (void)mqtt_disconnect(broker);
            ret = -1;
        }
    } else {
        ret = -1;
...
int mqtt_disconnect(mqtt_broker_handle_t* broker) {
	...
	// Send the packet
	if(broker->net_if->send(broker->socket_info, packet, sizeof(packet)) >= 0) {
#if 1 // broker sends RST?
        if (broker->net_if->close(broker) < 0) {
            return -1;
        }
#endif
    } else {
...
int mqtt_publish_with_qos(mqtt_broker_handle_t* broker, const char* topic, const char* msg, uint8_t retain, uint8_t qos, uint16_t* message_id) {
	...
	uint8_t fixed_header_size = 2;    // Default size = one byte Message Type + one byte Remaining Length
    uint8_t var_header_size = topiclen + 2 + qos_size;

	uint16_t remainLen = var_header_size + msglen;
	if (remainLen > 127) {
		fixed_header_size++;          // add an additional byte for Remaining Length
	}

	// Variable header
	uint8_t *var_header   = malloc(var_header_size); // Topic size (2 bytes), utf-encoded topic
	uint8_t *fixed_header = malloc(fixed_header_size);
    int packet_len = fixed_header_size + var_header_size + msglen;
	uint8_t *packet       = malloc(packet_len);
    if (!var_header || !fixed_header || !packet) {
        if (var_header) free(var_header);
        if (fixed_header) free(fixed_header);
        if (packet) free(packet);
        return -1;
    }

	memset(var_header, 0, var_header_size);
   ...
   if (remainLen <= 127) {
	...
	memcpy(packet+fixed_header_size, var_header, var_header_size);
	memcpy(packet+fixed_header_size+var_header_size, msg, msglen);

	// Send the packet
    int ret = 0;
	if(broker->net_if->send(broker->socket_info, packet, packet_len) < 0) {
		ret = -1;
	}
...
int mqtt_subscribe(mqtt_broker_handle_t* broker, const char* topic, uint16_t* message_id) {
	size_t topiclen = strlen(topic);
    uint8_t utf_topic_len = topiclen + 3;// Topic size (2 bytes), utf-encoded topic, QoS byte
    int packet_len;
	...
	// utf topic
	uint8_t *utf_topic = malloc(utf_topic_len);
    packet_len = sizeof(var_header)+sizeof(fixed_header)+utf_topic_len;
	uint8_t *packet = malloc(sizeof(var_header)+sizeof(fixed_header)+utf_topic_len);
    if (!utf_topic || !packet) {
        if (utf_topic) free(utf_topic);
        if (packet) free(packet);
        return -1;
    }
    ...
    // Wait for SUBACK
    if (broker->net_if->recv(broker->socket_info, RecvBuf, MQTT_RECV_BUF_SIZE, MQTT_RECV_TIMEOUT) >= 0) {
        if (MQTTParseMessageType(RecvBuf) != MQTT_MSG_SUBACK) {
            ret = -1;
        }
    }
    free(utf_topic);
...
int mqtt_unsubscribe(mqtt_broker_handle_t* broker, const char* topic, uint16_t* message_id) {
	...
	// utf topic
	uint8_t *utf_topic = malloc(utf_topic_len); // Topic size (2 bytes), utf-encoded topic
    packet_len = sizeof(var_header)+sizeof(fixed_header)+utf_topic_len;
	uint8_t *packet = malloc(packet_len);
    if (!utf_topic || !packet) {
        if (utf_topic) free(utf_topic);
