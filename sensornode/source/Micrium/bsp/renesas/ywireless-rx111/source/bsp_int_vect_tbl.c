...
*          2) In either case BSP_IntVectSet() must becalled to point to BSP_IntVectTbl[]
*
*********************************************************************************************************
*/

#if     BSP_CFG_INT_VECT_TBL_RAM_EN > 0u
__root  static  CPU_FNCT_VOID  BSP_IntVectTbl[] @ ".inttable" =
#else
__root  const   CPU_FNCT_VOID  BSP_IntVectTbl[] @ ".inttable" =
#endif
{
...
#if BSP_CFG_USBD_EN > 0u
    (CPU_FNCT_VOID)USBD_BSP_RX111_IntHandler,       /*  36, USB0  D0FIFO0                               */
    (CPU_FNCT_VOID)USBD_BSP_RX111_IntHandler,       /*  37, USB0  D1FIFO0                               */
    (CPU_FNCT_VOID)USBD_BSP_RX111_IntHandler,       /*  38, USB0  USBI0                                 */
#else
...
#ifdef RX231
#if BSP_CFG_NET_EN > 0u
    (CPU_FNCT_VOID)QCA_BSP_ISR_YWireless_RX111_GT202,   /*  64 IRQ0 */
#else
...
#else  // RX111
    (CPU_FNCT_VOID)BSP_IntHandler_065,              /*  65 IRQ1 */
    (CPU_FNCT_VOID)BSP_SwitchIRQ2,                  /*  66 IRQ2 */
    (CPU_FNCT_VOID)BSP_SwitchIRQ3,                  /*  67 IRQ3 */
    (CPU_FNCT_VOID)BSP_SwitchIRQ4,                  /*  68 IRQ4 */
#endif
#ifdef RX231
    (CPU_FNCT_VOID)BSP_IntHandler_069,              /*  69 IRQ5 */
    (CPU_FNCT_VOID)BSP_SwitchIRQ2,                  /*  70 IRQ6 */
    (CPU_FNCT_VOID)BSP_SwitchIRQ3,                  /*  71 IRQ7 */
#else
#if BSP_CFG_NET_EN > 0u
    (CPU_FNCT_VOID)QCA_BSP_ISR_YWireless_RX111_GT202,   /*  69 IRQ5 */
#else
    (CPU_FNCT_VOID)BSP_IntHandler_069,              /*  69 IRQ5 */
#endif
#ifdef M1_BSP_IRQ_PMOD3
    (CPU_FNCT_VOID)M1_BSP_PMOD3,                    /*  70 IRQ6 */
#else
    (CPU_FNCT_VOID)BSP_IntHandler_070,              /*  70 IRQ6 */
#endif
#ifdef M1_BSP_IRQ_PMOD1
    (CPU_FNCT_VOID)M1_BSP_PMOD1,                    /*  71 IRQ7 */
#else
...
#if BSP_CFG_USBD_EN > 0u
    (CPU_FNCT_VOID)USBD_BSP_RX111_IntHandler,       /*  90, USB0  USBR0                                 */
#else
...
#else
    (CPU_FNCT_VOID)M1_BSP_UART_ERR,                 /* 214, ERI0 Interrupt */
    (CPU_FNCT_VOID)M1_BSP_UART_RX,                  /*      215 --------------------------------------- */
#endif
...
#else
    // SCI1 interrupts (ERI1, RX1, TXI1) are currently only enabled in UART mode, and so these handlers are somewhat UART specific
    (CPU_FNCT_VOID)M1_BSP_UART_ERR1,                 /* 218, ERI1 Interrupt */
    (CPU_FNCT_VOID)M1_BSP_UART_RX1,                  /*      219 --------------------------------------- */
    (CPU_FNCT_VOID)BSP_IntHandler_220,              /*      220 --------------------------------------- */
#endif
...
#else
    // SCI12 interrupts (ERI1, RX1, TXI1) are currently only enabled in UART mode, and so these handlers are somewhat UART specific
    (CPU_FNCT_VOID)M1_BSP_UART_ERR12,                 /* 238, ERI12 Interrupt */
    (CPU_FNCT_VOID)M1_BSP_UART_RX12,                  /*      239 --------------------------------------- */
#endif
