...
* Programmer(s) : BAN
*********************************************************************************************************
* Note(s)       : (1) The LPC24XX SD/MMC card interface is described in documentation from NXP :
*
*                         UM10237.  LPC24XX User manual.  Rev.03 -- 15 January 2009.  "Chapter 21:
*                         LPC24XX SD/MMC card interface".
*
*                 (2) LPC24XX_CLK_FREQ should be set to the value of the LPC23XX's core clock frequency.
*********************************************************************************************************
*/
...
#define  LPC2XXX_RAM_BUF_ADDR                     0x7FD02000u
...
#define  LPC2XXX_BIT_MCIPOWER_CTRL_OFF                  0x00u
#define  LPC2XXX_BIT_MCIPOWER_CTRL_UP                   0x02u
#define  LPC2XXX_BIT_MCIPOWER_CTRL_ON                   0x03u
#define  LPC2XXX_BIT_MCIPOWER_CTRL_MASK                 0x03u
...
*                   ----------------------------------------------
*
*                       * Power supply enable for external power supply.  Jumper on board selects between
*                         positive and negative polarity for signal from MCU.  This driver assumes MCIPWR
*                         is configured active HIGH.
*                      ** Jumper on board selects between P1.6 and P0.22.  This driver assumes MCIDAT0 is
*                         configured P1.6.
*
*               (3) The LPC24XX DMA can ONLY access the internal USB RAM and external memories on any of
*                   the four dynamic or four static memory banks.  This driver uses the upper 8-kB of the
*                   USB RAM (0x7FD02000-0x7FD03FFF).
*********************************************************************************************************
*/
...
CPU_BOOLEAN  FSDev_SD_Card_BSP_Open (FS_QTY  unit_nbr)
{
    ...
    if ((LPC2XXX_REG_MCIPOWER & LPC2XXX_BIT_MCIPOWER_CTRL_MASK) != LPC2XXX_BIT_MCIPOWER_CTRL_OFF) {
        LPC2XXX_REG_MCIPOWER = LPC2XXX_BIT_MCIPOWER_CTRL_OFF;
    }
    ...
    LPC2XXX_REG_SCS        &= ~LPC2XXX_BIT_SCS_MCIPWR;
                                                                ...
                                                                /* Cfg P1[2,3,5..7,11,12] as MCICLK, MCICMD, ...        */
                                                                /* ... MCIPWR, MCIDAT0, MCIDAT1, MCIDAT2, MCIDAT3.      */
    pinsel                  =  LPC2XXX_REG_PINSEL2;
    pinsel                 &= ~0x03C0FCF0u;
    pinsel                 |=  0x0280A8A0u;
    LPC2XXX_REG_PINSEL2     =  pinsel;
    ...
    LPC2XXX_REG_MCIPOWER    = LPC2XXX_BIT_MCIPOWER_CTRL_UP;     /* Power up.                                            */
    while ((LPC2XXX_REG_MCIPOWER & LPC2XXX_BIT_MCIPOWER_CTRL_MASK) == LPC2XXX_BIT_MCIPOWER_CTRL_OFF) {
        ;
    }
    for (i = 0u; i < 1000u; i++) {
        ;
    ...
    LPC2XXX_REG_MCIPOWER   |= LPC2XXX_BIT_MCIPOWER_CTRL_ON;     /* Power on.                                            */
    for (i = 0u; i < 1000u; i++) {
        ;
    }
...
void  FSDev_SD_Card_BSP_Close (FS_QTY  unit_nbr)
{
    ...
    LPC2XXX_REG_MCIPOWER = LPC2XXX_BIT_MCIPOWER_CTRL_OFF;       /* Power off.                                           */
   ...
   (void)&p_data;
    ...
    if (p_cmd->DataDir == FS_DEV_SD_CARD_DATA_DIR_CARD_TO_HOST) {
        ...
        switch (p_cmd->BlkSize) {                               /* Set blk size.                                        */
            ...
            case 512u:
            default:     datactrl |= LPC2XXX_BIT_MCIDATACTRL_BLOCKSIZE_0512_BYTE;    break;
        }
        ...
        LPC2XXX_REG_GPDMA_DMACC1DESTADDR      = (CPU_INT32U)p_data;
                                   ...
                                   FS_DEV_SD_CARD_ERR  *p_err)
{
    CPU_INT32U   i;
    CPU_BOOLEAN  ok;
    CPU_BOOLEAN  dma_busy;
    CPU_BOOLEAN  timeout_not_reached;
   ...
   (void)&unit_nbr;                                             /* Prevent compiler warning.                            */
    ...
    ok = FS_OS_SemPend(&FSDev_SD_Card_OS_Sem, LPC2XXX_WAIT_TIMEOUT_ms);
                                                                ...
                                                                /* Return error code if timeout occurs                  */
    if (ok == DEF_FAIL) {
    ...
    i                   = 0u;
    dma_busy            = DEF_BIT_IS_CLR(LPC2XXX_REG_GPDMA_DMACRAWINTTCSTATUS, DEF_BIT_01);
    timeout_not_reached = DEF_YES;

    while ((dma_busy == DEF_YES) && (timeout_not_reached == DEF_YES)) {
        FS_OS_Dly_ms ((CPU_INT16U)1u);
        dma_busy            = DEF_BIT_IS_CLR(LPC2XXX_REG_GPDMA_DMACRAWINTTCSTATUS, DEF_BIT_01);
        timeout_not_reached = (i<LPC2XXX_WAIT_TIMEOUT_ms);
        i++;
    }
