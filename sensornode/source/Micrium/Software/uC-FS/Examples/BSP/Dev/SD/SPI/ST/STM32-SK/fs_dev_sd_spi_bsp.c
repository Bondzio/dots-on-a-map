...
*
*                         ST STM32F103VB ON THE IAR STM32-SK EVALUATION BOARD
*
...
*                         and STM32F103xx advanced ARM-based 32-bit MCUs.  Rev 6, September 2008.
...
#define  STM32_PCLK1_CLK_FREQ_MHZ                     36000000uL

#define  STM32_PIN_SD_CS                            DEF_BIT_12  /* GPIOB.12                                             */
#define  STM32_PIN_SPI2_SCK                         DEF_BIT_13  /* GPIOB.13                                             */
#define  STM32_PIN_SPI2_MISO                        DEF_BIT_14  /* GPIOB.14                                             */
#define  STM32_PIN_SPI2_MOSI                        DEF_BIT_15  /* GPIOB.15                                             */
...
*                   -----------------------------------------------
*                   |    SPI2_NSS   |     PB[12]    | CS  (pin 1) |
*                   |    SPI2_MOSI  |     PB[13]    | DI  (pin 2) |
*                   |    SPI2_SCK   |     PA[14]    | CLK (pin 5) |
*                   |    SPI2_MISO  |     PA[15]    | D0  (pin 7) |
*                   -----------------------------------------------
*********************************************************************************************************
*/
...
static  CPU_BOOLEAN  FSDev_BSP_SPI_Open (FS_QTY  unit_nbr)
{
    ...
    STM32_REG_RCC_APB1RSTR &= ~DEF_BIT_14;
    STM32_REG_RCC_APB1ENR  |=  DEF_BIT_14;                      /* En SPI2 clk.                                         */

    STM32_REG_RCC_APB2RSTR &= ~DEF_BIT_03;
    STM32_REG_RCC_APB2ENR  |=  DEF_BIT_03;                      /* En GPIOB clk.                                        */



                                                                /* --------------------- CFG GPIO --------------------- */
                                                                /* Cfg SCK, MOSO, MISO as alt fnct push-pull.           */
    temp                 = STM32_REG_GPIOB_CRH;                 /* Cfg SD CS as general purpose output.                 */
    temp                &= 0x0000FFFFu;
    temp                |= 0xBBB30000u;
    STM32_REG_GPIOB_CRH  = temp;



                                                                /* ---------------------- INIT SPI -------------------- */
    STM32_REG_SPI2_CR1  = STM32_BIT_SPI_CR1_MSTR                /* Master.                                              */
                        | STM32_BIT_SPI_CR1_BR_MASK             /* Max baud rate div.                                   */
                        | STM32_BIT_SPI_CR1_SSI
                        | STM32_BIT_SPI_CR1_SSM;                /* Software slave management.                           */

    STM32_REG_SPI2_CR2  = 0u;
    STM32_REG_SPI2_CR1 |= STM32_BIT_SPI_CR1_SPE;                /* Enable SPI.                                          */
                                ...
                                CPU_SIZE_T   cnt)
{
   ...
   (void)&unit_nbr;                                             /* Prevent compiler warning.                            */
    ...
    while (cnt > 0u) {
        STM32_REG_SPI2_DR = 0xFFu;                              /* Tx dummy byte.                                       */

                                                                /* Wait to rx byte.                                     */
        do {
            rxd = DEF_BIT_IS_SET(STM32_REG_SPI2_SR, STM32_BIT_SPI_SR_RXNE);
        } while (rxd == DEF_NO);

        datum     = STM32_REG_SPI2_DR;                          /* Rd rx'd byte.                                        */
       *p_dest_08 = datum;
        p_dest_08++;
        cnt--;
    }
                                ...
                                CPU_SIZE_T   cnt)
{
   ...
   (void)&unit_nbr;                                             /* Prevent compiler warning.                            */
    ...
    while (cnt > 0u) {
        datum             = *p_src_08;
        STM32_REG_SPI2_DR =  datum;                             /* Tx byte.                                             */
        p_src_08++;
        ...
        datum = STM32_REG_SPI2_DR;                              /* Rd rx'd dummy byte.                                  */
       (void)&datum;
...
static  void  FSDev_BSP_SPI_ChipSelEn (FS_QTY  unit_nbr)
{
   (void)&unit_nbr;                                             /* Prevent compiler warning.                            */
    ...
    STM32_REG_GPIOB_BSRR = DEF_BIT_28;
}
...
static  void  FSDev_BSP_SPI_ChipSelDis (FS_QTY  unit_nbr)
{
   (void)&unit_nbr;                                             /* Prevent compiler warning.                            */
    ...
    STM32_REG_GPIOB_BSRR = DEF_BIT_12;
}
                                        ...
                                        CPU_INT32U  freq)
{
   ...
   (void)&unit_nbr;                                             /* Prevent compiler warning.                            */
    ...
    clk_div     =  2u;
    clk_div_log =  1u;
    clk_div_min = (STM32_PCLK1_CLK_FREQ_MHZ + freq - 1u) / freq;
    ...
    STM32_REG_SPI2_CR1 &= ~STM32_BIT_SPI_CR1_SPE;               /* Dis SPI.                                             */

    STM32_REG_SPI2_CR1 &= ~STM32_BIT_SPI_CR1_BR_MASK;           /* Clr baudrate div ...                                 */
    STM32_REG_SPI2_CR1 |= ((clk_div_log - 1u) << 3);            /* ... set new baudrate div.                            */
    STM32_REG_SPI2_CR1 |=  STM32_BIT_SPI_CR1_SPE;               /* Re-en SPI.                                           */
}
