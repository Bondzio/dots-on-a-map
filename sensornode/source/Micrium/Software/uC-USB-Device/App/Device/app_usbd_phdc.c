...
#define  APP_USBD_PHDC_OS_III_EN                          DEF_ENABLED
#else
#define  APP_USBD_PHDC_OS_III_EN                          DEF_DISABLED
#endif
...
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)                    /* ---------------------- OS-III ---------------------- */
    OS_TMR               Tmr;
#else                                                           /* ---------------------- OS-II ----------------------- */
    OS_TMR              *Tmr;
#endif
...
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)                    /* ---------------------- OS-III ---------------------- */
static  OS_TCB               App_USBD_PHDC_TxCommTaskTCB[APP_CFG_USBD_PHDC_MAX_NBR_TASKS];
static  OS_TCB               App_USBD_PHDC_RxCommTaskTCB;
static  OS_Q                 App_USBD_PHDC_ItemQ[APP_CFG_USBD_PHDC_MAX_NBR_TASKS];
#else                                                           /* ---------------------- OS-II ----------------------- */
...
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)                    /* ---------------------- OS-III ---------------------- */
#if  (OS_CFG_TMR_TASK_RATE_HZ < 100u)
#error  "OS_CFG_TMR_TASK_RATE_HZ illegally #define'd in 'os_cfg_app.h' [MUST be >= 100]"
#endif

#if  (OS_CFG_STAT_TASK_EN == DEF_DISABLED)
#error  "OS_CFG_STAT_TASK_EN illegally #define'd in 'os_cfg.h' [MUST be DEF_ENABLED]"
#endif

#if  (OS_CFG_TMR_EN == DEF_DISABLED)
#error  "OS_CFG_TMR_EN illegally #define'd in 'os_cfg.h' [MUST be DEF_ENABLED]"
#endif

#if  (OS_CFG_TMR_DEL_EN == DEF_DISABLED)
#error  "OS_CFG_TMR_DEL_EN illegally #define'd in 'os_cfg.h' [MUST be DEF_ENABLED]"
#endif
                                 ...
                                 CPU_INT08U  cfg_fs)
{
...
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)                    /* ---------------------- OS-III ---------------------- */
    p_dflt_item->Data[7u]      =  OSStatTaskCPUUsage;
#else                                                           /* ---------------------- OS-II ----------------------- */
    ...
    OSTmrCreate(           &p_dflt_item->Tmr,                   /* Create tmr for dflt item.                            */
                (CPU_CHAR *)0,
                            tmr_period,
                (OS_TICK   )0,
                            OS_OPT_TMR_ONE_SHOT,
...
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)                    /* ---------------------- OS-III ---------------------- */
        OSQCreate(&App_USBD_PHDC_ItemQ[cnt],
...
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)                    /* ---------------------- OS-III ---------------------- */
    OSTaskCreate(                 &App_USBD_PHDC_RxCommTaskTCB,
                                  "USB Device PHDC rx comm",
                                   App_USBD_PHDC_RxCommTask,
                 (void *)(CPU_ADDR)App_USBD_PHDC_ClassNbr,
...
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)                    /* ---------------------- OS-III ---------------------- */
        OSTaskCreate(                 &App_USBD_PHDC_TxCommTaskTCB[cnt],
                                      "USBD PHDC tx comm",
    ...
    while (DEF_TRUE) {
...
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)
        ...
        switch (data[0u]) {                                     /* First data octets is the cmd.                        */
...
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)                    /* ---------------------- OS-III ---------------------- */
                     tmr_period = ((((OS_TICK)p_item->Period * OSCfg_TmrTaskRate_Hz)  + 1000u - 1u) / 1000u);
                     OSTmrCreate(           &p_item->Tmr,
                                 (CPU_CHAR *)0,
                                             tmr_period,
                                 (OS_TICK   )0,
                                             OS_OPT_TMR_ONE_SHOT,
            ...
            case APP_USBD_PHDC_CMD_RESET:                       /* Host send reset cmd when app start.                  */
                 ...
                 for (cnt = 0; cnt < APP_CFG_USBD_PHDC_MAX_NBR_TASKS; cnt ++) {
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)                    /* ---------------------- OS-III ---------------------- */
                     (void)OSQFlush(&App_USBD_PHDC_ItemQ[cnt],
                                    &os_err);
#else                                                           /* ---------------------- OS-II ----------------------- */
...
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)                    /* ---------------------- OS-III ---------------------- */
            p_item->Data[7u] =  OSStatTaskCPUUsage;
#else                                                           /* ---------------------- OS-II ----------------------- */
                                         ...
                                         void  *p_arg)
{
    ...
    nbr    =  CPU_CntTrailZeros08(p_item->LatencyRely);
                                                                ...
                                                                /* Post item on Q.                                      */
#if (APP_USBD_PHDC_OS_III_EN == DEF_ENABLED)                    /* ---------------------- OS-III ---------------------- */
    OSQPost(       &App_USBD_PHDC_ItemQ[nbr],
            (void *)p_item,
                    0u,
                    OS_OPT_POST_FIFO,
                   &os_err);
