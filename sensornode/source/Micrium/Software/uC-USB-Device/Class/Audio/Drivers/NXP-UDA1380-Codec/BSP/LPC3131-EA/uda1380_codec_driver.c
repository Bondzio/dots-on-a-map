...
static const struct uda1380_setup pb_rec_enable[] =
{
  ...
  {UDA1380_REG_EVALCLK,  (REG_EVALCLK_EN_ADC |
                          REG_EVALCLK_EN_DEC |
                          REG_EVALCLK_EN_DAC |
                          REG_EVALCLK_EN_INT)},

  {UDA1380_REG_PWRCTRL,  REG_PWRCTRL_PON_BIAS},
  {UDA1380_REG_PWRCTRL, (REG_PWRCTRL_PON_DAC  |
                         REG_PWRCTRL_PON_BIAS |
                         REG_PWRCTRL_PON_LNA  |
                         REG_PWRCTRL_PON_PGAR)},
                                                                /* PON_PGAL = 0; PON_PGAR = PON_ADCR = 0.               */
  {UDA1380_REG_PWRCTRL, (REG_PWRCTRL_PON_HP   |
                         REG_PWRCTRL_PON_DAC  |
                         REG_PWRCTRL_PON_BIAS |
                         REG_PWRCTRL_PON_LNA  |
                         REG_PWRCTRL_PON_PGAL |
                         REG_PWRCTRL_PON_ADCL)},
  {UDA1380_REG_PWRCTRL, (REG_PWRCTRL_PON_HP   |
                         REG_PWRCTRL_PON_DAC  |
                         REG_PWRCTRL_PON_BIAS |
                         REG_PWRCTRL_PON_LNA  |
  ...
  {UDA1380_REG_EVALCLK,  (REG_EVALCLK_EN_ADC |
                          ...
                          REG_EVALCLK_EN_DAC |
                          REG_EVALCLK_EN_INT)},
  {UDA1380_REG_MSTRMUTE, 0x0800},
  {UDA1380_REG_EVALCLK,  (REG_EVALCLK_EN_ADC |
                          REG_EVALCLK_EN_DEC |
                          REG_EVALCLK_EN_DAC |
                          REG_EVALCLK_EN_INT)},
  {UDA1380_REG_MIXVOL,   0x3c00},
  {UDA1380_REG_PGA,      0x0000},
  {UDA1380_REG_ADC,     (REG_ADC_SEL_LNA     |                  /* Mic on left ch selected.                             */
                         REG_ADC_SEL_MIC     |                  /* Mic input routed also to right ch.                   */
                         REG_ADC_SKIP_DCFIL  |
                         REG_ADC_EN_DCFIL)},
  {UDA1380_REG_AGC,    0x0000},                                 /* AGC_EN = 0.                                          */
...
static INT_32 i2c_dev_xfer(UNS_8 *buf, UNS_32 len, UNS_32 read)
{
      ...
      _NO_ERROR)
  {
    ...
    status = i2c_mstr_ioctl(i2chandle, I2C_MSTR_GET_STATUS, 0);
    while (((status & (I2C_MSTR_STATUS_ARBF | I2C_MSTR_STATUS_NOACKF | I2C_MSTR_STATUS_DONE)) == 0) && (retry > 0u))
    {
      status = i2c_mstr_ioctl(i2chandle,
                              I2C_MSTR_GET_STATUS, 0);
      retry--;
...
/***********************************************************************
 *
 * Function: uda1380_playback_mute
 *
 ...
 *
 **********************************************************************/
void uda1380_playback_mute(INT_32 mute)
{
    if (mute) {
        uda1380_set(UDA1380_REG_MSTRMUTE, (MSTRMUTE_MTM_EN | MSTRMUTE_MT2_EN | MSTRMUTE_MT1_EN));
    } else {
        uda1380_clear(UDA1380_REG_MSTRMUTE, (MSTRMUTE_MTM_EN | MSTRMUTE_MT2_EN | MSTRMUTE_MT1_EN));
    }
}

/***********************************************************************
 *
 * Function: uda1380_playback_set_boost
 *
 ...
 *
 **********************************************************************/
void uda1380_playback_set_boost(UNS_8 bass, UNS_8 treble)
{
  bass = bass & 0x7;
  treble = treble & 0x3;
...
/***********************************************************************
 *
 * Function: uda1380_playback_set_volume
 *
 ...
 *
 **********************************************************************/
void uda1380_playback_set_volume(UNS_8 left_vol, UNS_8 right_vol)
{
 ...
 *
 **********************************************************************/
void uda1380_record_mute(INT_32 mute)
{
    if (mute) {
        uda1380_set(UDA1380_REG_PGA, UDA1380_PGA_MT_ADC);
    } else {
        uda1380_clear(UDA1380_REG_PGA, UDA1380_PGA_MT_ADC);
    }
}

/***********************************************************************
 *
 * Function: uda1380_record_set_volume
 *
 * Purpose: Set volume level
 *
 * Processing:
 ...
 *
 * Parameters:
 *     left_vol  : Left volume value, 48 (max) to 128 (min)
 *     right_vol : Right volume value, 48 (max) to 128 (min)
 *
 * Outputs: None
 *
 * Returns: Nothing
 ...
 *
 **********************************************************************/
void uda1380_record_set_volume(UNS_8 left_vol, UNS_8 right_vol)
{
    uda1380_write(UDA1380_REG_DECVOL, ((left_vol << 8) | right_vol));
}
