...
;********************************************************************************************************
    ...
    .ref     OS_CPU_IntHandler                                   ; Chip Support/BSP specific exception handler.
...
OS_CPU_IntHandlerAddr:     .word  OS_CPU_IntHandler
...
OS_CPU_ARM_FP_RegPushSkip?
                          PUSH    {rx}                          ;     ... Save Floating point exception register.
                          PUSH    {rx}
...
OS_CPU_ARM_ExceptHndlr_BrkTask:
    ...
    LDR     R1, OSIntNestingCtrAddr                             ;   OSIntNestingCtr++;
    LDRB    R2, [R1]
    ADD     R2, R2, #1
    STRB    R2, [R1]
    ...
    LDR     R1, OSTCBCurPtrAddr                                 ;   OSTCBCurPtr->StkPtr = SP;
    LDR     R2, [R1]
    STR     SP, [R2]
...
OS_CPU_ARM_ExceptHndlr_BrkTask_1:
    ...
    LDR     R1, OS_CPU_IntHandlerAddr                           ; OS_CPU_IntHandler(src_id = R0);
    MOV     LR, PC
    BX      R1
...
OS_CPU_ARM_ExceptHndlr_BrkExcept:
    ...
    STMFD   SP!, {R3}
                                                                ...
                                                                ; HANDLE NESTING COUNTER:
    LDR     R2, OSIntNestingCtrAddr                             ;   OSIntNestingCtr++;
    LDRB    R4, [R2]
    ADD     R4, R4, #1
    STRB    R4, [R2]
    ...
    MSR     CPSR_cxsf, R3                                       ; RESTORE INTERRUPTED MODE.
                                                                ...
                                                                ; EXECUTE EXCEPTION/INTERRUPT HANDLER:
    LDR     R2, OS_CPU_IntHandlerAddr                           ; OS_CPU_IntHandler(src_id = R0);
    MOV     LR, PC
    BX      R2
...
OS_CPU_ARM_IRQHndlr_BreakIRQ:
    ...
    STR     SP, [R3]
                                                                ...
                                                                ; EXECUTE EXCEPTION/INTERRUPT HANDLER:
    LDR     R3, OS_CPU_IntHandlerAddr                           ; OS_CPU_IntHandler(src_id = R0)
    MOV     LR, PC
    BX      R3
