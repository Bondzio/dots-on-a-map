...
void  OS_TLS_Init (OS_ERR *p_err)
{
    ...
    OSMutexCreate((OS_MUTEX *)&OS_TLS_NewLib_MallocMutex,
                  (CPU_CHAR *)"TLS Malloc Mtx",
                  (OS_ERR   *)p_err);

    OSMutexCreate((OS_MUTEX *)&OS_TLS_NewLib_EnvMutex,
                  (CPU_CHAR *)"TLS Env    Mtx",
                  (OS_ERR   *)p_err);
}
...
static  void  OS_TLS_NewLib_EnvUnlock (void)
{
    ...
    OSMutexPost((OS_MUTEX *)&OS_TLS_NewLib_EnvMutex,
                (OS_OPT    )OS_OPT_POST_NONE,
                (OS_ERR   *)&err);
}
