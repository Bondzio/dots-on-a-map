...
#if ENABLE_P2P_MODE
static A_STATUS wmi_p2p_goneg_result_event_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap,A_UINT8 len);

static A_STATUS wmi_p2p_list_peristent_network(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT8 len);

static A_STATUS wmi_p2p_req_auth_event_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap,A_UINT32 len);

static A_STATUS wmi_p2p_node_list_event_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT32 len);

static A_STATUS wmi_p2p_invite_sent_result_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT8 len);
static A_STATUS wmi_p2p_prov_disc_resp_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT8 len);
static A_STATUS wmi_p2p_prov_disc_req_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT8 len);
#if 1//KK

static A_STATUS wmi_p2p_sdpd_event_rx(struct wmi_t *wmip, A_UINT8 devId, 
                    A_UINT8 *datap, A_UINT8 len);
static A_STATUS wmi_p2p_invite_req_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap,
                                           A_UINT8 len);
static A_STATUS wmi_p2p_invite_rcvd_result_rx(struct wmi_t *wmip, A_UINT8 devId, 
                    A_UINT8 *datap, A_UINT8 len);
#endif
...
static A_STATUS wmi_test_event_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT32 len);
#endif
static A_STATUS wmi_errorEvent_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, int len);
...
wmi_dix_2_dot3(struct wmi_t *wmip, void *osbuf)
{
    ...
    macHdr.typeOrLen = (A_UINT16)A_CPU2BE16(A_NETBUF_LEN(osbuf) - sizeof(ATH_MAC_HDR) +
...
A_STATUS
wmi_meta_add(struct wmi_t *wmip, void *osbuf, A_UINT8 *pVersion,void *pTxMetaS)
{
...
typedef PREPACK struct _iphdr {
    A_UINT8     ip_ver_hdrlen FIELD_PACKED;          /* version and hdr length */
    A_UINT8     ip_tos FIELD_PACKED;                 /* type of service */
    A_UINT16    ip_len FIELD_PACKED;                 /* total length */
    A_UINT16    ip_id FIELD_PACKED;                  /* identification */
    A_INT16     ip_off FIELD_PACKED;                 /* fragment offset field */
#define IP_DF 0x4000                    /* dont fragment flag */
#define IP_MF 0x2000                    /* more fragments flag */
#define IP_OFFMASK 0x1fff               /* mask for fragmenting bits */
    A_UINT8     ip_ttl FIELD_PACKED;                 /* time to live */
    A_UINT8     ip_p FIELD_PACKED;                   /* protocol */
    A_UINT16    ip_sum FIELD_PACKED;                 /* checksum */
    A_UINT8     ip_src[4] FIELD_PACKED;              /* source and dest address */
    A_UINT8     ip_dst[4] FIELD_PACKED;
} POSTPACK iphdr;

static A_UINT8
wmi_determine_userPriority(A_UINT8 *pkt, A_UINT32 layer2Pri)
{
    A_UINT8 ipPri;
...
static A_STATUS
wmi_test_ev_length(A_UINT16 id, A_INT32 len)
{
    A_UINT16 i;
    A_CONST struct{
        A_UINT16 id;
 ...
 */
A_STATUS
wmi_control_rx_xtnd(struct wmi_t *wmip, A_UINT8 devId, void *osbuf)
{
    WMIX_CMD_HDR *cmd;
    A_UINT16 id;
    A_UINT8 *datap;
...
wmi_control_rx(struct wmi_t *wmip, void *osbuf)
{
    ...
    union{
        A_VOID      *void_ev; //used for initialization only
    ...
    datap = A_NETBUF_DATA(osbuf);
    VOID_EV = datap;
    len = (A_INT32)A_NETBUF_LEN(osbuf);
    ...
    case (WMI_REPORT_STATISTICS_EVENTID):
        A_WMI_TARGETSTATS_EVENT(wmip->wmi_devt, devId, datap, (A_UINT32)len);
        break;
...
#endif
    default:
       if(A_OK != (status = A_CUSTOM_WMI_EVENT(id, wmip->wmi_devt, datap, len, osbuf))) {
         A_DPRINTF(DBG_WMI|DBG_ERROR,
            (DBGFMT "Unknown id 0x%x\n", DBGARG, id));
         wmip->wmi_stats.cmd_id_err++;
         status = A_ERROR;
 ...
 */
static A_STATUS
wmi_errorEvent_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, int len)
{
               ...
               WMI_SYNC_FLAG syncflag)
{
    ...
    cHdr->commandId = A_CPU2LE16((A_UINT16) cmdId);
...
wmi_ap_set_param(struct wmi_t *wmip, A_VOID *data)
{
    ...
    switch (ap_param->cmd_subset)
	{
        ...
        case AP_SUB_CMD_INACT_TIME:
		{
			...
			stackU.conn_inact_cmd.period = A_CPU2LE32(*(A_UINT32 *)(ap_param->data));
        ...
        case AP_SUB_CMD_BCON_INT:
		{
			...
			stackU.beacon_int_cmd.beaconInterval = A_CPU2LE16((*(A_UINT16 *)(ap_param->data)));
        ...
        case AP_SUB_CMD_DTIM_INT:
		{
			...
			stackU.set_dtim_cmd.dtim = *(A_UINT8 *)(ap_param->data);
        ...
        case AP_SUB_CMD_PSBUF:
		{
			...
			stackU.psbuf_offload_cmd.enable = ((A_UINT8 *)ap_param->data)[0];
			stackU.psbuf_offload_cmd.psBufCount = ((A_UINT8 *)ap_param->data)[1];
...
static A_STATUS
wmi_p2p_goneg_result_event_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT8 len)
{
...
static A_STATUS
wmi_p2p_list_peristent_network(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT8 len)
{
...
static A_STATUS
wmi_p2p_req_auth_event_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap,A_UINT32 len)
{
...
static A_STATUS
wmi_p2p_node_list_event_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT32 len)
{
...
static A_STATUS
wmi_p2p_prov_disc_resp_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT8 len)
{
...
static A_STATUS
wmi_p2p_prov_disc_req_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT8 len)
{
...
#if 1//KK
static A_STATUS
wmi_p2p_sdpd_event_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap, A_UINT8 len)
{
...
static A_STATUS
wmi_p2p_invite_req_rx(struct wmi_t *wmip, A_UINT8 devId, A_UINT8 *datap,
                                           A_UINT8 len)
{
...
static A_STATUS wmi_p2p_invite_sent_result_rx(struct wmi_t *wmip, A_UINT8 devId, 
                    A_UINT8 *datap, A_UINT8 len)
{
...
A_UINT8  reverse_credits_init = 0;

A_STATUS wmi_reverse_credit_cmd (A_VOID* handle, A_BOOL enable, A_UINT8 *endpoints, A_UINT8 *credits)
{
	A_STATUS status = A_ERROR;
    A_UINT8    i;
    ...
    do {
        ...
        else if(status == A_NO_MEMORY)
        {
                ...
                if(A_OK != CUSTOM_DRIVER_WAIT_FOR_CONDITION(p_Global_Cxt, &(pDCxt->tx_complete_pend), A_FALSE, 5000))
                {
                    A_ASSERT(0);
                }
         }
	...
	Custom_Driver_WakeDriver(p_Global_Cxt);
    A_MDELAY(10);
...
#define  CLASS_MAPPING_LEN(n)   (sizeof(A_UINT32) * n + n)

A_STATUS wmi_rcv_data_classifier_cmd (A_VOID* handle, A_UINT8 offset, A_UINT8 shift, A_UINT32 mask, A_UINT8 count,
                                      A_UINT32 *category_mapping, A_UINT8 *ep_mapping)
{
	A_STATUS status = A_ERROR;
    ...
    memcpy(DataCls.category_mapping_data, category_mapping, sizeof(A_UINT32) * count);
...
A_STATUS wmi_update_reverse_credits_cmd (A_VOID* handle, A_UINT8 endpoint, A_UINT32 nCredits)
{
  	A_STATUS status = A_ERROR;
...
A_STATUS wmi_dset_host_cfg_cmd (A_VOID* handle)
{
	A_STATUS status = A_ERROR;
    A_UINT8    i;
