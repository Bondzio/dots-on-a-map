...
//#define AR4100_BUFFER_SIZE                ((2 *(sizeof(A_NETBUF)+1664) - sizeof(A_NETBUF) - 196 - 100) / 2 - sizeof(A_NETBUF)) & (~3)
//#else
...
typedef struct {
	...
	A_UINT16 serviceID;
	A_UINT32 intCredits;
    A_UINT16 intCreditCounter;
    A_UINT16 rptCreditCounter;
    A_UINT8 epIdx;
	A_UINT8 rsvd;
    A_UINT16 credits;
    A_UINT16 maxCredits;
...
typedef struct _a_driver_context
{
    ...
    A_BOOL          testResp;
#endif
    A_UINT8         rxMultiBufferStatus; /* maintains available status of rx buffers */
    A_BOOL			pfmDone;/* scan done indicator, set by WMI_SCAN_COMPLETE_EVENTID */
    A_BOOL          wps_in_progress;
    A_UINT32        pwrStateSetting;
    A_BOOL			dset_op_done;        /* dset op done indicator, set by WMI_SCAN_COMPLETE_EVENTID */
    A_UINT8          raw_temperature ; /*register value */
    A_UINT32         tempDegree;  /*temperature degree*/
    A_BOOL          temperatureValid;
}A_DRIVER_CONTEXT;
