...
A_VOID 
Driver_ReportRxBuffStatus(A_VOID *pCxt, A_BOOL status, A_UINT8 epid);
#else
A_VOID 
...
A_STATUS
Driver_SubmitEp0TxRequest(A_VOID *pCxt, A_VOID *pReq);
A_BOOL 
...
A_STATUS  
Driver_StoreRecallFirmwareDownload(A_VOID *pCxt);
...
A_BOOL
Hcd_ReadCPUInterrupt(A_VOID *pCxt, A_UINT8 *cpuIntrCause);
...
A_STATUS
Api_SetPowerMode(A_VOID *pCxt,POWER_MODE *pwrmode);
