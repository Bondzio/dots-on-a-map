...
typedef PREPACK struct sock_ip_bridgemode {
    WMI_SOCKET_CMD wmi_cmd FIELD_PACKED; 
    A_UINT16 bridgemode FIELD_PACKED;
...
typedef PREPACK struct httpc_command {
    ...
    A_UINT8   url[64] FIELD_PACKED;
    A_UINT8   data[128] FIELD_PACKED;
...
typedef PREPACK struct sock_ota_upgrade {
    WMI_SOCKET_CMD wmi_cmd FIELD_PACKED;
    A_UINT32 ipaddress FIELD_PACKED;
    A_CHAR   filename[128] FIELD_PACKED;
    A_UINT8  mode FIELD_PACKED;
    A_UINT8  preserve_last FIELD_PACKED;
    A_UINT8  protocol FIELD_PACKED;
...
typedef PREPACK struct sock_get_dev_from_dest_ip {
    ...
    union 
    {
        A_UINT32   ipaddr FIELD_PACKED;
        IP6_ADDR_T ip6addr FIELD_PACKED;	
    }addr;
    A_UINT8 domain FIELD_PACKED;		//socket domain
}POSTPACK SOCK_GET_DEV_FROM_DEST_T;

#define SOCK_EV_MASK_SET(_pcxt, _cmd)\
{\
    if((_cmd) <= 31)\
        (_pcxt)->sock_st_mask |= 1<<(_cmd);\
    else\
        (_pcxt)->sock_st_mask2 |= 1<<((_cmd) - 32);\
}

#define SOCK_EV_MASK_CLEAR(_pcxt, _cmd) \
{\
    if((_cmd) <= 31)\
        (_pcxt)->sock_st_mask &= ~(1<<(_cmd));\
    else\
...
#define SOCK_EV_MASK2_TEST(_pcxt, _cmd) (((_pcxt)->sock_st_mask2 & (1<<(_cmd)))? 1:0)
...
A_INT32 Api_ipbridgemode(void* handle, A_UINT16 status);
...
A_INT32 Api_tcp_connection_timeout(A_VOID *pCxt,A_UINT32 timeout_val);

A_INT32 Api_ota_upgrade(A_VOID *pCxt,A_UINT32 addr,A_CHAR *filename,A_UINT8 mode,A_UINT8 preserve_last,A_UINT8 protocol,A_UINT32 * resp_code,A_UINT32 *length);
A_INT32 Api_ota_read_area(A_VOID *pCxt,A_UINT32 offset,A_UINT32 size,A_UCHAR *buffer,A_UINT32 *retlen);
A_INT32 Api_ota_done(A_VOID *pCxt);

#if ENABLE_ROUTING_CMDS
A_INT32 Api_ipv4_route(A_VOID *pCxt, A_UINT32 command, A_UINT32* ipv4_addr, A_UINT32* subnetMask, A_UINT32* gateway, A_UINT32* ifIndex, IPV4_ROUTE_LIST_T *routelist);
A_INT32 Api_ipv6_route(A_VOID *pCxt, A_UINT32 command, A_UINT8* ipv6_addr, A_UINT32* prefixLen, A_UINT8* gateway, A_UINT32* ifIndex, IPV6_ROUTE_LIST_T *routelist);
#endif

A_INT32 Api_ip_dns_server(A_VOID *pCxt,A_INT32 command);
#if ENABLE_SSL
