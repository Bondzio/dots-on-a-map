...
A_VOID *a_netbuf_alloc(A_INT32 size)
{
    ...
    p_a_netbuf = Mem_PoolBlkGet((MEM_POOL *)&(GET_DRIVER_CXT(p_ctx)->A_NetBufPool),
...
A_VOID a_netbuf_free(A_VOID* buffptr)
{
    ...
    switch (p_a_netbuf->pool_id) {
        ...
        case A_TX_NET_POOL:
             ...
             NetIF_TxDeallocTaskPost((CPU_INT08U *) p_a_netbuf->dealloc, 
                                                   &err_net);
    ...
    Mem_PoolBlkFree((MEM_POOL *)&(GET_DRIVER_CXT(p_ctx)->A_NetBufPool),
                                 p_a_netbuf,
                                &err_lib);
}
                 ...
                 A_UINT8 id)
{
	...
	addr = Mem_HeapAlloc((CPU_SIZE_T  ) size,
                         (CPU_SIZE_T  ) 4,
                         (CPU_SIZE_T *)&reqd_octets,
                         (LIB_ERR    *)&lib_err);
