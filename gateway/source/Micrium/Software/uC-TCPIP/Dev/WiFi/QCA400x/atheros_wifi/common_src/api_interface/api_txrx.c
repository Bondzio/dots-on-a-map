...
#if !ENABLE_STACK_OFFLOAD                          
	                tid = (A_UINT8)WMI_DATA_HDR_GET_UP(dhdr);
	                seq_no = (A_UINT16)(WMI_DATA_HDR_GET_SEQNO(dhdr));
#endif                        
...
#define AC_NOT_MAPPED   99
    ...
    do{
        ...
        if( (A_FALSE == pDCxt->conn[0].isConnected && A_FALSE == pDCxt->conn[0].isConnectPending)
            ...
            CUSTOM_DRIVER_WAIT_FOR_CONDITION(pCxt, &(pDCxt->conn[0].isConnected), A_TRUE, 5000);    	
