...
Driver_Main(A_VOID *pCxt, A_UINT8 scope, A_BOOL *pCanBlock, A_UINT16 *pBlock_msec)
{
    ...
    if(pDCxt->hcd.CpuInterrupt){
        ...
        if (Hcd_ReadCPUInterrupt(pCxt, &cpuIntrCause)) {
          pDCxt->hcd.CpuInterruptCause = cpuIntrCause;
        }
    	pDCxt->driver_state = DRIVER_STATE_INTERRUPT_PROCESSING;
...
A_UINT8 GetQueueIndexByEPID(A_UINT8 epid)
{
    A_UINT8 i;
...
A_UINT8 GetQueueCtrlIndexByEPID(A_UINT8 epid)
{
    A_UINT8 i;
