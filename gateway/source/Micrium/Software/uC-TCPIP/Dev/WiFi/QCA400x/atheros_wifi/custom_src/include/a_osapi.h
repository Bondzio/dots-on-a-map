...
#define A_MEMCPY(dst, src, len)         Mem_Copy((A_UINT8 *)(dst), (src), (len))
...
#define A_MUTEX_LOCK(mutex)             (a_mutex_lock((mutex)))
#define A_MUTEX_UNLOCK(mutex)           (a_mutex_unlock((mutex)))
#define A_IS_MUTEX_VALID(mutex)          TRUE
#define A_MUTEX_DELETE(mutex)           (a_mutex_destroy((mutex)))

/*
 * A_MDELAY - used to delay specified number of milliseconds.
 */
#define A_MDELAY(msecs)                 KAL_Dly((msecs))
...
extern void assert_func(A_UINT32);
#define A_ASSERT(expr) if(!(expr)) do{ assert_func(__LINE__);}while(0)
//#define A_ASSERT(expr) if(!(expr)) do{ ((A_CUSTOM_DRIVER_CONTEXT*)p_Global_Cxt)->line = __LINE__; assert_func(((A_CUSTOM_DRIVER_CONTEXT*)p_Global_Cxt)->line);}while(0)
