...
A_UINT8  trailor[sizeof(HTC_RECORD_HDR) + 6 * sizeof(HTC_CREDIT_REPORT)];
...
#if USE_16BIT_CREDIT_COUNTER
        pDCxt->ep[2].maxCredits = pDCxt->ep[2].credits = (A_UINT16)numCredits;
#endif
...
HTC_ProcessCpuInterrupt(A_VOID *pCxt)
{
	...
	A_DRIVER_CONTEXT *pDCxt = GET_DRIVER_COMMON(pCxt);
        ...
        if (pDCxt->hcd.CpuInterruptCause & 0x1) {
        ...
        if (pDCxt->hcd.CpuInterruptCause & 0x2) {
          //printf("SOF intr\n");
        }

        /* All interrupts have been processed */
        pDCxt->hcd.CpuInterruptCause = 0x0;
...
static int CollectCredits(A_UINT8 *buff, int lenFreeTrail)
{
    HTC_RECORD_HDR  *hd;
...
Htc_SendPacket(A_VOID *pCxt, A_VOID *pReq)
{
	...
	do{
        ...
        if (reverse_credits_init != 0)
        {
            ...
            A_UINT8  buff[sizeof(HTC_RECORD_HDR) + 6 * sizeof(HTC_CREDIT_REPORT)];
            A_UINT8  *data;
        ...
        len = (A_UINT16)A_NETBUF_LEN(pReq) - HTC_HDR_LENGTH;
#if 0                                                           /* Micrium modifications.                               */
	    A_SET_UINT16_FIELD(pHdrBuf, HTC_FRAME_HDR, PayloadLen, len);
#endif
        pHdrBuf[2] = (A_UINT8) (len & 0x00FF);
        pHdrBuf[3] = (A_UINT8)((len & 0xFF00) >> 8);
