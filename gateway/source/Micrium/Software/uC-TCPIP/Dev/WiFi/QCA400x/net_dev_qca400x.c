...
#include  "atheros_wifi/custom_src/include/cust_netbuf.h"
#include  "atheros_wifi/common_src/include/common_api.h"
#include  "atheros_wifi/custom_src/include/custom_api.h"
#include  "atheros_wifi/atheros_wifi_api.h"
#include  "atheros_wifi/common_src/include/wmi_api.h"
#include  "atheros_wifi/common_src/wmi/wmi_host.h"
#include  "atheros_wifi/include/htc.h"
#include  "atheros_wifi/custom_src/include/a_osapi.h"
#include  "atheros_wifi/include/wmi.h"
...
#define NET_DEV_DRIVER_WAKE_EVENT                           "NetDev Driver Wake Event"
#define NET_DEV_DRIVER_START_EVENT                          "NetDev Driver Start Event"
#define NET_DEV_USER_WAKE_EVENT                             "NetDev User Wake Event"
...
*********************************************************************************************************
*/
const  NET_DEV_API_WIFI  NetDev_API_QCA400X = {                                  /* Ether PIO dev API fnct ptrs :       */
...
*                           NET_DEV_ERR_INVALID_CFG     Device Configuration error.
*
*
* Return(s)   : none.
*
...
*
*               (8) Drivers SHOULD validate device configuration values and set *p_err to
                           ...
                           NET_ERR  *p_err)
{
    ...
    p_dev_data->CommonCxt = Mem_HeapAlloc( sizeof(A_DRIVER_CONTEXT),
                                           sizeof(CPU_INT32U),
                                          &reqd_octets,
                                          &lib_err);
    if (p_dev_data->CommonCxt == (void *)0) {
       *p_err = NET_DEV_ERR_MEM_ALLOC;
        return;
    }
    ...
    Mem_PoolCreate(&p_dev_data->A_NetBufPool,
                    0,
                    0,
                    NET_DEV_NB_A_NETBUF,
                    nbytes,
    ...
    p_dev_data->DriverRxQueue = KAL_QCreate((const CPU_CHAR *) NET_DEV_DRIVER_RX_QUEUE,
    ...
    if (err_kal != KAL_ERR_NONE) {
       *p_err = NET_DEV_ERR_INIT;
                                                                ...
                                                                /* -------------- CREATE DRIVER SEMAPHORES -------------- */
                                                                /* Create Atheros driver wake semaphore.                  */
    p_dev_data->DriverWakeSemHandle = KAL_SemCreate((const  CPU_CHAR *) NET_DEV_DRIVER_WAKE_EVENT,
                                                                        DEF_NULL,
                                                                       &err_kal);
    if (err_kal != KAL_ERR_NONE) {
       *p_err = NET_DEV_ERR_INIT;
        return;
                                                                 ...
                                                                 /* Create Atheros user wake semaphore.                    */
    p_dev_data->UserWakeSemHandle = KAL_SemCreate((const  CPU_CHAR *) NET_DEV_USER_WAKE_EVENT,
                                                                      DEF_NULL,
                                                                     &err_kal);
    if (err_kal != KAL_ERR_NONE) {
       *p_err = NET_DEV_ERR_INIT;
    ...
    p_dev_data->DriverStartSemHandle = KAL_SemCreate((const  CPU_CHAR *) NET_DEV_DRIVER_START_EVENT,
                                                                         DEF_NULL,
                                                                        &err_kal);
    if (err_kal != KAL_ERR_NONE) {
       *p_err = NET_DEV_ERR_INIT;
    ...
    p_dev_data->GlobalBufSemHandle = KAL_SemCreate((const  CPU_CHAR *) NET_DEV_WMI_BUF_SEM,
                                                                       DEF_NULL,
                                                                      &err_kal);
    if (err_kal != KAL_ERR_NONE) {
       *p_err = NET_DEV_ERR_INIT;
...
*
*               (6) The Qualcomm/Atheros driver don't allow to configure the MAC addr. The only method
                            ...
                            NET_ERR  *p_err)
{
    ...
    valid = NetIF_AddrHW_IsValidHandler(p_if->Nbr, p_dev_data->DeviceMacAddr, &err);
...
*
* Note(s)     : (1) If a receive error occurs, the function SHOULD return 0 for the size, a
...
*
*               (2) Reading data from the device hardware may occur in various sized reads :
*
...
*
*               (3) A pointer set equal to pbuf_new and sized according to the required data
*                   read size determined in (2) should be used to read data from the device
*                   into the receive buffer.
*
*               (4) Some devices may interrupt only ONCE for a recieved frame.  The driver MAY need
                                  ...
                                  NET_DEV_ISR_TYPE   type)
{
   ...
   (void)&type;                                                 /* Prevent possible 'variable unused' warnings.         */
}
             ...
             if (p_dev_data->WaitRespType == NET_IF_WIFI_CMD_JOIN ) {
                ...
                if (p_wmi_disconnect->disconnectReason == DISCONNECT_CMD) {
                 ...
                 if (valid == DEF_YES) {
                     ...
                     p_if->Link = NET_IF_LINK_DOWN;             /* Dissassociation event                                */
                 }
   ...
   *p_err                     = NET_DEV_ERR_NONE;
    ...
    switch (cmd) {
        ...
        case NET_IF_WIFI_CMD_GET_PEER_INFO:
             ...
             return (ctn * sizeof(NET_IF_WIFI_PEER));
   ...
   *p_err      = NET_DEV_ERR_NONE;
    ...
    switch (cmd) {
        ...
        case NET_IF_IO_CTRL_LINK_STATE_GET_INFO:
             ...
             p_ctx->MgmtCompleted     = DEF_YES;
             p_dev_data->WaitRespType = NET_DEV_MGMT_NONE;
            *p_buf_rtn                = p_dev_data->LinkStatus ;
             rtn                      = 1;
             break;
...
*
* Note(s)     : (1) This function call Api_ParseInfoElem() which is part of the Qualcomm/Atheros Driver.
*
*********************************************************************************************************
*/
...
*               p_err       Pointer to variable  that will receive the return error code from this function :
*
*                                   NET_DEV_ERR_NONE        Connect command is successfully sent
*                                   NET_DEV_ERR_FAULT       Connect command failed.
*
* Return(s)   : none.
*
* Caller(s)   : NetDev_MgmtExecuteCmd().
*
* Note(s)     : (1) This function call Api_ConnectWiFi() which is part of the Qualcomm/Atheros Driver.
*
*********************************************************************************************************
*/
   ...
   *p_err      = NET_DEV_ERR_NONE;
    ...
    if (cmd == NET_IF_WIFI_CMD_JOIN) {
        if (p_ap_cfg->NetType == NET_IF_WIFI_NET_TYPE_INFRASTRUCTURE) {
            ...
            *p_err = NET_DEV_ERR_NOT_SUPPORTED;
             return;
        }
    ...
    p_dcxt->conn[p_dcxt->devId].ssidLen = Str_Len_N(p_ap_cfg->SSID.SSID,
...
*
* Description : Set the header for the RX Queue and deliver to the RX Task.
*
* Argument(s) : p_if        Pointer to a network interface.
*
*               p_a_netbuf  Pointer on the A_NETBUF struct containing the packet information.
*
*               type        Type of the packet to be queued.
...
*               p_err       Pointer to variable  that will receive the return error code from this function :
*
*                                   NET_DEV_ERR_NONE        The packet has been successfully delivered to the RX task.
*                                   NET_DEV_ERR_FAULT       The packet has not been delivered properly to the RX task.
*
* Return(s)   : none.
*
* Caller(s)   : Custom_Api_RxComplete().
*
* Note(s)     : (1) If the buffer came from the RX Net buffer pool, set the delivered buffer flag. This
                                ...
                                NET_ERR                *p_err)
{
    ...
    if (p_a_netbuf->pool_id == A_RX_NET_POOL) {                 /* Set the buffer as delivered. See Note 1.             */
        p_a_netbuf->RxBufDelivered = DEF_YES;
                                                                ...
                                                                /* Wake the RX Task.                                    */
    NetIF_RxTaskSignal(p_if->Nbr, &net_err);
...
*
* Description : Validate the packet received and its type then deliver it to the RX task.
*
* Argument(s) : p_cxt    Pointer to a network interface. (equivalent to NET_IF *p_if)
*
*               p_req    Pointer on the A_NETBUF struct that describe the received packet.
*
* Return(s)   : none.
*
* Caller(s)   : Api_RxComplete() via 'ath_custom_init.Api_RxComplete()'.
*
* Return(s)   : none.
*
                              ...
                              A_VOID* p_req)
{
    ...
    if (p_dcxt->wmiEnabled == A_TRUE) {
        if (ept == ENDPOINT_1) {                                /* -------------------- MGMT PACKET ------------------ */
            ...
            do{
                if ((p_dcxt->conn[p_dcxt->devId].networkType != AP_NETWORK        ) && /* See note 2a.                                      */
...
*                                          NetDev_WMICmdSend()
*
* Description : Send a WMI command request to the Driver task and wait to be completely send.
*
* Argument(s) : pCxt    Pointer to a network interface. (equivalent to NET_IF *p_if)
*
*               cmd     ID of the WMI command.
*
*               pParam  Pointer to a parameters struct of the command to send.
*
*               length  Length in byte of the parameters structure.
*
* Return(s)   : Status code
*
*                       A_OK only, otherwise any error should block (A_ASSERT)
*
*
* Caller(s)   : NetDev_MgmtExecuteCmd(),
*               NetDev_MgmtExecuteCmdConnect(),
*               NetDev_MgmtExecuteCmdScan().
*
* Note(s)     : (1) This function call wmi_cmd_start() which is part of the Qualcomm/Atheros Driver.
*
*********************************************************************************************************
*/
...
*                                           NetDev_GetEventID()
*
* Description : Get the WMI Event ID from a packet.
*
* Argument(s) : p_if    Pointer to a network interface.
*
*               p_buf   Pointer at the beginning of the buffer to be parsed.
*
* Return(s)   : Value of the WMI Event Id.
*
* Caller(s)   : NetDev_MgmtDemux(),
*               NetDev_MgmtProcessResp().
*
* Note(s)     : (1) This function don't validate the Event ID .
*
*********************************************************************************************************
*/
...
*
* Description : Get and Initialize a A_NETBUF structure for a Tramsmit Request to the driver.
*
* Argument(s) : p_if    Pointer to a network interface.
*
*               p_data  Pointer to a packet to send.
*
*               size    Size of the packet to send.
*
*               p_err   Pointer to variable  that will receive the return error code from this function :
*
*                               NET_DEV_ERR_NONE        The ANetBuf has been successfully acquired
*                               NET_DEV_ERR_FAULT       The operation failed.
*
* Return(s)   : A_NETBUF struture initialized with p_data pointer and size value.
*
* Caller(s)   : NetDev_Tx().
*
* Note(s)     : (1) The Qualcomm/Atheros Driver use the A_NETBUF structure to describe the buffer for
*                   TX/RX. This is the structure that is use for transmit request to the driver.
*
*               (2) Once the transmit request is deliver to the driver task, it will free the A_NETBUF
                                            ...
                                            NET_ERR    *p_err)
{
    ...
    p_a_netbuf = (A_NETBUF *) Mem_PoolBlkGet((MEM_POOL *) &(GET_DRIVER_CXT(p_if)->A_NetBufPool),
                                             (CPU_SIZE_T)  sizeof(A_NETBUF),
                                             (LIB_ERR  *) &err_lib);
    if (err_lib != LIB_MEM_ERR_NONE) {
       *p_err = NET_DEV_ERR_FAULT;
        return (A_NETBUF *)DEF_NULL;
    }
                                                                /* --------------- INITIALIZE A_NETBUF ---------------- */
    if (p_a_netbuf != NULL) {
...
*                                         NetDev_AddPeerInfo()
*
* Description : Add a new peer in the list in the dev data area.
*
* Argument(s) : p_if    Pointer to a network interface.
*
*               p_data  Pointer to a connect event with the peer info.
*
*               p_err   Pointer to variable  that will receive the return error code from this function :
*
*                               NET_DEV_ERR_NONE    The peer info is successfully added to the peer list.
*
* Return(s)   : none.
*
* Caller(s)   : NetDev_MgmtDemux().
                               ...
                               NET_ERR    *p_err)
{
    ...
    p_buf = (NET_DEV_PEER_INFO *)Mem_DynPoolBlkGet(&p_dev_data->PeerInfoPool,
                                                   &err_lib);
    if (err_lib != LIB_MEM_ERR_NONE) {
      *p_err = NET_DEV_ERR_FAULT;
       return;
...
* Argument(s) : p_if    Pointer to a network interface.
*
*               p_data  Pointer to a disconnect event with the peer info.
*
*               p_err   Pointer to variable  that will receive the return error code from this function :
*
*                               NET_DEV_ERR_NONE    The peer info is successfully deleted from the peer list.
*
* Return(s)   : none.
*
* Caller(s)   : NetDev_MgmtDemux().
...
*                                         NetDev_GetPeerInfo()
*
* Description : Get all the information of the active peers in the the peer list..
*
* Argument(s) : p_if            Pointer to a network interface.
*
*               p_data          Pointer to return the information of the active peer.=s.
*
*               buf_max_len     The maximum buffer length of p_data
*
*               p_err           Pointer to variable that will receive the return error code from this function :
*
*                               NET_DEV_ERR_NONE    The peer info is successfully deleted from the peer list.
*
*
* Return(s)   : Number of active peer in the peer list.
*
* Caller(s)   : NetDev_MgmtExecuteCmd().
*
* Note(s)     : none.
...
*
* Description : Get the pointer to the event data from the a net buffer.
*
* Argument(s) : p_if    Pointer to a network interface.
*
*               p_buf   Pointer to the buffer to parse.
*
* Return(s)   : Pointer to the event data
*
* Caller(s)   : NetDev_AddPeerInfo(),
*               NetDev_DeletePeerInfo().
*
* Note(s)     : none.
*
