                                     ...
                                     NET_IF_NBR     if_nbr)
{
    ...
    if (success != DEF_OK) {
        return (NET_SOCK_BSD_ERR_OPEN);
                                                                ...
                                                                /* ------------ SET SOCK TO BLOCKING MODE ------------- */
    success = NetSock_CfgBlock(sock_id, NET_SOCK_BLOCK_SEL_BLOCK, &err_net);
    if (success != DEF_OK) {
        return (NET_SOCK_BSD_ERR_OPEN);
    }
                                      ...
                                      DHCPc_ERR      *perr)
{
    ...
    while ((nego_retry_cnt <  DHCPc_NEGO_MAX_RETRY) &&
           (nego_done      != DEF_YES)) {
...
* Caller(s)   : DHCPc_InitStateHandler().
*
* Note(s)     : (2) RFC #2131, section 'Constructing and sending DHCP messages', states that "selecting
...
*                   the transaction ID.
*
*               (3) When the function returns DHCPc_ERR_NONE, the OFFER message's parameters are copied
*                   into the structure pointed to by 'pif_info' so that a REQUEST message can be crafted.
*
*               (4) If NO DHCP OFFER is received following a DHCP DISCOVER transmission, the caller is
                              ...
                              DHCPc_ERR      *perr)
{
    ...
    while ((discover_retry_cnt <  DHCPc_NEGO_MAX_RETRY) &&      /* While DISCOVER retry < max retry ...                 */
           (discover_done      != DEF_YES)) {                   /* ... & DISCOVER NOT done,         ...                 */
        ...
        if (discover_dly == DEF_YES) {                          /* Dly DISCOVER, if req'd.                              */
            ...
            KAL_Dly(dly_ms);
                                                                ...
                                                                /* ------------------ GENERATE 'XID' ------------------ */
        pif_info->TransactionID++;                              /* Inc last transaction ID (see Note #2).               */
                             ...
                             DHCPc_ERR      *perr)
{
    ...
    while ((request_retry_cnt <  DHCPc_NEGO_MAX_RETRY) &&       /* While REQUEST retry < max retry ...                  */
           (request_done      != DEF_YES)) {                    /* ... & REQUEST NOT done,         ...                  */
                                      ...
                                      DHCPc_ERR      *perr)
{
    ...
    cfgd = NetIPv4_CfgAddrAddDynamic((NET_IF_NBR   ) pif_info->IF_Nbr,
                                     (NET_IPv4_ADDR) addr_host,
                                     (NET_IPv4_ADDR) NET_IPv4_ADDR_LOCAL_LINK_HOST_MAX,
                                     (NET_IPv4_ADDR) NET_IPv4_ADDR_NONE,
                                     (NET_ERR     *)&err_net);
    if (cfgd != DEF_OK) {
       *perr = DHCPc_ERR_IF_CFG;
                                               ...
                                               CPU_INT08U   addr_hw_len)
{
    ...
    seed_hw_addr = ((((CPU_INT32U) paddr_hw + (addr_hw_len - 1)) << 24) |
                    (((CPU_INT32U) paddr_hw + (addr_hw_len    )) << 16));
