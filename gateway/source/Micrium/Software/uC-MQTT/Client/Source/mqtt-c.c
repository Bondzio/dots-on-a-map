//TODO : In app examples, remove unusesd parameter set.
/*
*********************************************************************************************************
*                                               uC/MQTTc
...
typedef  struct  mqttc_data {
    ...
    const  MQTTc_CFG     *CfgPtr;                               /* Ptr to cfg passed at init.                           */
           KAL_Q_HANDLE   MsgQ_Handle;                          /* Handle to msg Q.                                     */
...
static  void         MQTTc_Task            (void            *p_data);
...
static  MQTTc_MSG   *MQTTc_MsgPend         (void);
                         ...
                         MQTTc_ERR     *p_err)
{
    ...
    kal_feat_is_ok &= KAL_FeatureQuery(KAL_FEATURE_Q_CREATE,    KAL_OPT_NONE);
    kal_feat_is_ok &= KAL_FeatureQuery(KAL_FEATURE_Q_POST,      KAL_OPT_POST_NONE);
    kal_feat_is_ok &= KAL_FeatureQuery(KAL_FEATURE_Q_PEND,      KAL_OPT_PEND_NON_BLOCKING);
    ...
    if (err_lib != LIB_MEM_ERR_NONE) {
       *p_err = MQTTc_ERR_ALLOC;
        return;
                                                                ...
                                                                /* Create msg Q.                                        */
    p_temp_mqttc_data->MsgQ_Handle = KAL_QCreate("MQTTc Msg Queue",
                                                  p_cfg->MaxMsgNbr,
                                                  DEF_NULL,
                                                 &err_kal);
    if (err_kal != KAL_ERR_NONE) {
...
*                                          MQTTc_ConnClose()
*
* Description : Close a MQTTc Connection.
*
                         ...
                         MQTTc_ERR         *p_err)
{
    ...
    switch (type) {
        case MQTTc_PARAM_TYPE_MSG_BUF_PTR:
             p_msg->BufPtr = (CPU_INT08U *)p_param;
             break;
...
*
*               payload_str     String containing the payload to publish.  Must stay valid until the
                            ...
                            MQTTc_ERR     *p_err)
{
   ...
   *p_buf = (CPU_INT08U)(str_len & 0xFFu);
    ...
    str_len = Str_Len(payload_str);                             /* Copy payload.                                        */
    Str_Copy((CPU_CHAR *)p_buf, payload_str);
                                  ...
                                  MQTTc_ERR    *p_err)
{
   ...
   *p_buf_start = topic_nbr;
    ...
    MQTTc_DBG_GLOBAL_BUF_COPY(p_msg->BufPtr, 10u);
    ...
    if ((rem_len + ((CPU_INT16U)(p_buf - p_msg->BufPtr))) > p_msg->BufLen) {
                                                                /* Confirm msg fits in provided buf.                    */
       *p_err = MQTTc_ERR_INVALID_BUF_SIZE;
        return;
    }
   ...
   *p_buf = (CPU_INT08U)(msg_id & 0xFFu);
    ...
    p_msg->BufPtr  =  p_buf_start;                              /* Adjust BufPtr to start of content to send.           */
                     ...
                     MQTTc_ERR   *p_err)
{
    ...
    p_buf = MQTTc_FixedHdrBufCfg(p_msg->BufPtr,                 /* Cfg fixed hdr section of msg.                        */
...
static  void  MQTTc_Task (void  *p_arg)
{
    ...
    while (DEF_TRUE) {
        ...
        if (MQTTc_Ptr->ConnHeadPtr != DEF_NULL) {
            ...
            KAL_Dly(DEF_MAX(1u, MQTTc_Ptr->CfgPtr->TaskDly));   /* In this case the task must absolutely dly.           */
        }

        MQTTc_MsgProcess();

        KAL_Dly(MQTTc_Ptr->CfgPtr->TaskDly);
    }
}
    ...
    if (p_msg->State == MQTTc_MSG_STATE_MUST_TX) {              /* If msg needs to be tx'd, tx it.                      */
        ...
        switch (p_msg->Type) {
            ...
            case MQTTc_MSG_TYPE_DISCONNECT:
                 ...
                 p_conn->NextTxMsgTxLen += MQTTc_SockTx(p_conn,
                                                       &p_msg->BufPtr[p_conn->NextTxMsgTxLen],
                                                       (p_msg->XferLen - p_conn->NextTxMsgTxLen),
                                                       &p_msg->Err);
        ...
        switch (p_msg->Type) {
            ...
            case MQTTc_MSG_TYPE_SUBSCRIBE:                      /* Finished sending a SUBSCRIBE, wait to rx SUBACK.     */
                 ...
                 p_buf_topic_nbr = p_msg->BufPtr - 1u;
...
static  void  MQTTc_RdSockProcess (MQTTc_CONN  *p_conn)
{
    ...
    if (p_conn->NextMsgPtr == DEF_NULL) {                       /* If next msg is already known, skip this step.        */
        ...
        if (p_conn->NextMsgLenIsCmpl == DEF_NO) {
            ...
            do {                                                /* Read rem len of msg. This can be a multi-byte field. */
                p_conn->NextMsgRxLen += MQTTc_SockRx(p_conn,
                                                    &rem_len,
                                                     1u,
                                                    &err_mqttc);
                if (err_mqttc == MQTTc_ERR_FATAL) {
                ...
                p_conn->NextMsgLen += (rem_len & MQTT_MSG_FIXED_HDR_REM_LEN_MSK);
        ...
        if ( (p_conn->NextMsgMsgID_IsCmpl == DEF_NO) &&
            ...
            p_conn->NextMsgRxLen = MQTTc_SockRx(p_conn,         /* Rx msg ID if msg has one.                            */
                                               &msg_id_rx[p_conn->NextMsgRxLen],
                                               (MQTT_MSG_ID_SIZE - p_conn->NextMsgRxLen),
                                               &err_mqttc);
            if (err_mqttc == MQTTc_ERR_FATAL) {
                goto err_remove_conn_close_sock;
            } else if (err_mqttc != MQTTc_ERR_NONE) {           /* Wait to be able to rx data to continue.              */
                if ((err_mqttc            == MQTTc_ERR_RX_BUF_EMPTY) &&
                    (p_conn->NextMsgRxLen == 1u)) {             /* Keep first part of msg ID rx'd.                      */
    ...
    if (p_conn->NextMsgLen != 0u) {                             /* If there is more than the hdr to rx, rx it.          */
        ...
        rx_len = MQTTc_SockRx(p_conn,
                             &p_next_msg->BufPtr[p_conn->NextMsgRxLen],
                              p_conn->NextMsgLen,
                             &err_mqttc);
    ...
    MQTTc_DBG_GLOBAL_BUF_COPY(p_next_msg->BufPtr, p_conn->NextMsgRxLen);
    ...
    if (p_next_msg->Type == MQTTc_MSG_TYPE_PUBLISH) {           /* Rx'd a Publish msg from broker.                      */
        ...
        if (p_next_msg->QoS == 0u) {                            /* If QoS is 0, msg is cmpl'd. Exec callback.           */
            ...
            len    = MQTT_MSG_UTF8_LEN_RD(&p_next_msg->BufPtr[MQTTc_PUBLISH_RX_MSG_BUF_OFFSET]) + MQTT_MSG_UTF8_LEN_SIZE;
            msg_id = MQTT_MSG_UTF8_LEN_RD(&p_next_msg->BufPtr[len + MQTTc_PUBLISH_RX_MSG_BUF_OFFSET]);
            ...
            p_buf = MQTTc_FixedHdrBufCfg(p_next_msg->BufPtr,    /* Cfg fixed hdr section of msg for reply.              */
                                         type,
                                         DEF_NO,
                                         0u,
                                         DEF_NO,
        ...
        switch (p_next_msg->Type) {
            case MQTTc_MSG_TYPE_CONNACK:
                 if ((p_conn->NextMsgRxLen   != 2u) ||
                     (p_next_msg->BufPtr[0u] != MQTT_MSG_VAR_HDR_CONNACK_RET_CODE_ACCEPTED)) {
            ...
            case MQTTc_MSG_TYPE_SUBACK:
                 ...
                 p_buf_topic_nbr = p_next_msg->BufPtr - 1u;
                 topic_nbr       = p_buf_topic_nbr[0u];

                 for (topic_ix = 0u; topic_ix < topic_nbr; topic_ix++) {
                     p_buf_topic_nbr--;
                     if ((*p_buf_topic_nbr) != p_next_msg->BufPtr[topic_ix]) {
                 ...
                 p_next_msg->BufPtr = p_buf_topic_nbr;
                 break;
            ...
            case MQTTc_MSG_TYPE_PUBREC:
                 ...
                 p_buf = MQTTc_FixedHdrBufCfg(p_next_msg->BufPtr,
...
static  void  MQTTc_MsgCallbackExec (MQTTc_MSG  *p_msg)
{
    ...
    if (p_msg != p_conn->PublishRxMsgPtr) {
        ...
        p_msg->ConnPtr->TxMsgHeadPtr = p_msg->NextPtr;          /* Msg is necessarily located at head of list.          */
        p_msg->NextPtr               = DEF_NULL;
        ...
        CPU_INT08U  *p_buf_start   = &p_msg->BufPtr[MQTTc_PUBLISH_RX_MSG_BUF_OFFSET];
...
static  MQTTc_MSG  *MQTTc_MsgPend (void)
{
    ...
    p_msg = (MQTTc_MSG *)KAL_QPend(MQTTc_Ptr->MsgQ_Handle,
                                   KAL_OPT_PEND_NON_BLOCKING,
                                   0u,
                                  &err_kal);
    if ((err_kal != KAL_ERR_NONE) &&
        (err_kal != KAL_ERR_WOULD_BLOCK)) {
        MQTTc_DBG_TRACE_INFO(("!!! ERROR !!! Failed to pend on queue. Err: %i\n\r", err_kal));
    }
                             ...
                             MQTTc_ERR       *p_err)
{
    ...
    KAL_QPost(        MQTTc_Ptr->MsgQ_Handle,                   /* Post msg to Q.                                       */
              (void *)p_msg,
                      KAL_OPT_POST_NONE,
                     &err_kal);
    if (err_kal == KAL_ERR_NONE) {
       ...
       *p_err = MQTTc_ERR_OS_FAIL;
        MQTTc_DBG_TRACE_INFO(("!!! ERROR !!! Failed to post on queue. Err: %i\n\r", err_kal));
    }
                                           ...
                                           MQTTc_ERR       *p_err)
{
    ...
    CPU_INT32U   encoded_len;
    CPU_INT32U   val;
    CPU_INT32U   len            = rem_len;
    CPU_INT08U   byte_bit_shift = 0u;
    ...
    encoded_len = 0u;                                       /* Encode rem_len with Continuation Bit, if needed.     */
    do {
        val          = (len) % (MQTT_MSG_FIXED_HDR_REM_LEN_MAX_LEN << byte_bit_shift);
        len         -=  val;
        encoded_len +=  val;

        if ((len) > 0u) {
            DEF_BIT_SET((encoded_len), (MQTT_MSG_FIXED_HDR_REM_LEN_CONTINUATION_BIT << byte_bit_shift));
        }
        byte_bit_shift += 8u;
    } while ((len) > 0u);

    do  {                                                   /* Write encoded len in buf.                            */
        p_cur_buf++;
       *p_cur_buf   = (encoded_len & 0xFFu);
        encoded_len =  encoded_len >> 8u;
    } while (encoded_len != 0u);
