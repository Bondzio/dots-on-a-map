...
CPU_BOOLEAN  AppDHCPc_Init (NET_IF_NBR  if_nbr)
{
    ...
    while (cfg_done != DEF_YES) {
        ...
        dhcp_status = DHCPc_ChkStatus(if_nbr, &err_dhcp);       /* DHCP_STATUS_CFGD == 0x02                             */
        switch (dhcp_status) {
    ...
    if (dhcp_status == DHCP_STATUS_FAIL) {
        ...
        ip          = NetASCII_Str_to_IPv4((CPU_CHAR *)APP_NET_CFG_IF_0_IP_ADDR_STR, &err_net);
        msk         = NetASCII_Str_to_IPv4((CPU_CHAR *)APP_NET_CFG_IF_0_MASK_STR,    &err_net);
        gateway     = NetASCII_Str_to_IPv4((CPU_CHAR *)APP_NET_CFG_IF_0_GATEWAY_STR, &err_net);
