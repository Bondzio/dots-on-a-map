 ...
 *
 * Copyright Mocana Corp 2003-2015. All Rights Reserved.
...
getUpTime(void)
{
    return (RTOS_deltaMS(&gStartTime, NULL));
}
...
static char *getcurrtime()
{
    ...
    sprintf(g_currtimestring, "%s.%ld", tmp, milliseconds);
...
DEBUG_CONSOLE_printError(sbyte4 errorClass, sbyte *pPrintString, sbyte4 value)
{
    ...
    DB_PRINT("%s%d%s", pPrintString, value, CRLF);
}
...
DEBUG_CONSOLE_printString3(sbyte4 errorClass, sbyte *pPrintString1, sbyte *pPrintString2, sbyte *pPrintString3)
{
    ...
    DB_PRINT("%s%s%s%s", pPrintString1, pPrintString2, pPrintString3, CRLF);
}
...
{
#if !defined(__RTOS_ANDROID__) && !defined(__UCOS_RTOS__)
    FILE   *fod;
    MSTATUS status  = OK;

    DB_PRINT("Switching output stream", 0);
    if (dboutput)
    {
        fclose(dboutput);
        dboutput = NULL;
...
DEBUG_CONSOLE_dump_data(ubyte *address, int size, int limit, int wsize,
                        char *title)
{
    ubyte4      i, repeat, wperline, psize;
    ...
    for (i = 0; i < repeat; i++)
    {
        if ((i % wperline) == 0)
        {
            ...
            *ap = 0;
            DB_PRINT("%s | %s\n", linebuf, ascbuf);
            lp  = linebuf;
            sprintf(lp, "%08X: ", (ubyte4)(unsigned long)paddress);
            lp += 10;
            ap  = ascbuf;
            *ap = 0;
        }
        if (1 == wsize)
        {
            sprintf(lp, "%02X ", *paddress);
            lp += 3;
            *ap++ = isprint(*paddress) ? *paddress : '.';
        }
        else if (2 == wsize)
        {
            sprintf(lp, "%04X ", *(ubyte2 *)paddress);
            lp += 5;
        }
        else
        {
            sprintf(lp, "%08X ", *(ubyte4 *)paddress);
            lp += 9;
        }
        paddress += wsize;
    }
...
#elif defined(__XVXWORKS_RTOS__)
extern void
DEBUG_CONSOLE_printf(const char *format, ...)
{
    va_list ap;
    ...
    v5 = va_arg(ap, int);
    v6 = va_arg(ap, int);
    logMsg(format, v1, v2, v3, v4, v5, v6);
    va_end(ap);
}
...
#else
        vfprintf(dboutput, format, valist);
#endif
        fflush(dboutput);
    }
    else
