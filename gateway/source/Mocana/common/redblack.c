 ...
 * Red-Black Tree
 *
 * Copyright Mocana Corp 2007-2014. All Rights Reserved.
...
REDBLACK_defaultGetNode(void *pAllocCookie, void **ppNewNode)
{
    ...
    return MOC_alloc(sizeof(redBlackNodeDescr), (void **)ppNewNode);
}
                   ...
                   void *pAllocCookie)
{
    ...
    if (NULL == (pNewTree = MALLOC(sizeof(redBlackTreeDescr))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }

    MOC_MEMSET((void *)pNewTree, 0x00, sizeof(redBlackTreeDescr));
                ...
                const void **ppRetKey)
{
    ...
    *ppRetKey = (RB_NULL == pNode) ? NULL : pNode->pKey;
...
REDBLACK_traverseListInit(redBlackTreeDescr *pTree, redBlackListDescr **ppRetListTracker)
{
    ...
    if (OK > (status = MOC_alloc(sizeof(redBlackListDescr), (void **)ppRetListTracker)))
        goto exit;
