...
#ifdef __ENABLE_MOCANA_PRAGMA_MARK__
#pragma mark osx
#endif
...
MIP_CONVERT_ADDR_NBO(struct SOCKADDR_IN * pTargetAddr,MOC_IP_ADDRESS moc_addr)
{
    ...
    MOC_MEMSET((ubyte *)pTargetAddr->SIN_ADDR.S_ADDR, 0x00, sizeof(struct SOCKADDR_IN));

    /* in order to transform this we need an array of 16 bytes instead of an array of 4 ubyte4 */

    MOC_MEMCPY(pTempAddr, MOCADDR(*moc_addr), MOCADDRSIZE);
...
MIP_CONVERT_MOC_ADDR_NBO(MOC_IP_ADDRESS moc_addr,  struct SOCKADDR_IN * addr )
{
    ...
    MOC_MEMSET((ubyte *)MOCADDR(*moc_addr), 0x00, MOCADDRSIZE);

    /* in order to transform this we need an array of 16 bytes instead of an array of 4 ubyte4 */

    MOC_MEMCPY(pTempAddr2, addr->SIN_ADDR.S_ADDR, MOCADDRSIZE);
    ...
    MOC_MEMCPY(MOCADDR(*moc_addr), pTempAddr, MOCADDRSIZE);
...
MIP_CONVERT_NBO_MADDR_FROM_BUFFER(MOC_IP_ADDRESS moc_addr , char * addr)
{
    ...
    MOC_MEMCPY(pTempAddr2, addr, MOCADDRSIZE);
...
MIP_CONVERT_NBO(MOC_IP_ADDRESS moc_addr)
{
    ...
    MOC_MEMSET((ubyte *)pTempAddr, 0x00, MOCADDRSIZE );
    /* We need to make a copy in order to work with ubytes instead of ubyte4s */
    MOC_MEMCPY(pTempAddr2, MOCADDR(*moc_addr), MOCADDRSIZE);
...
MIP_COPY_ADDRS(struct SOCKADDR_IN * pTargetAddr, MOC_IP_ADDRESS moc_addr)
{
    ...
    MOC_MEMCPY(pTargetAddr->SIN_ADDR.S_ADDR,MOCADDR(*moc_addr),MOCADDRSIZE);
...
MIP_COPY_MOCADDRS(MOC_IP_ADDRESS_S ** pTargetAddr,  struct SOCKADDR_IN * addr)
{
    ...
    MOC_MEMCPY(MOCADDR(**pTargetAddr), addr->SIN_ADDR.S_ADDR,MOCADDRSIZE );
...
MIP_COPY_MOCADDRS_CONSTRUCTOR(MOC_IP_ADDRESS pTargetAddr, MOC_IP_ADDRESS moc_addr)
{
    ...
    MOC_MEMCPY(MOCADDR(*pTargetAddr), MOCADDR(*moc_addr), MOCADDRSIZE);
...
MIP_COPY_ADDRS(struct SOCKADDR_IN * pTargetAddr, MOC_IP_ADDRESS_S * moc_addr)
{
    ...
    MOC_MEMCPY(&(*pTargetAddr).SIN_ADDR.S_ADDR,moc_addr,MOCADDRSIZE);
