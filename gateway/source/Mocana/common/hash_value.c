...
HASH_VALUE_hashGen(const void *pHashKeyData, ubyte4 hashKeyDataLength, ubyte4 initialHashValue, ubyte4 *pRetHashValue)
{
    ...
    if (0 == (((usize)(pHashKeyData)) & 0x3))
    {
        const ubyte4 *pHashKey = pHashKeyData;                           /* read 32-bit chunks */
    ...
    else if (0 == (((usize)(pHashKeyData)) & 0x1))
    {
        const ubyte2 *pHashKey = pHashKeyData;                           /* read 16-bit chunks */
        const ubyte  *k8;
    ...
    else
    {
        ...
        const ubyte *pHashKey = pHashKeyData;
