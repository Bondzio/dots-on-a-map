                             ...
                             funcPtrAllocElement pFuncAllocElement, funcPtrFreeElement pFuncFreeElement)
{
    ...
    if (NULL == (pHashTable = MALLOC(sizeof(hashTableIndices) + ((1 + hashTableSizeMask) * sizeof(hashTableIndexElement *)))))
    {
                           ...
                           funcPtrAllocHashPtrElement pFuncAllocElement, funcPtrFreeHashPtrElement pFuncFreeElement)
{
    ...
    if (NULL == (pHashTable = MALLOC(sizeof(hashTableOfPtrs) + ((1 + hashTableSizeMask) * sizeof(hashTablePtrElement *)))))
    {
        status = ERR_MEM_ALLOC_FAIL;
        goto exit;
    }
...
HASH_TABLE_deletePtr(hashTableOfPtrs *pHashTable, ubyte4 hashValue, void *pTestData,
                     MSTATUS(*funcPtrExtraMatchTest)(void * /* pAppData */, void * /* pTestData */, intBoolean * /* pRetIsMatch */),
                     void **ppRetAppDataToDelete, intBoolean *pRetFoundHashValue)
{
    ...
    *pRetFoundHashValue = FALSE;
    ...
    if (pBucket->hashValue == hashValue)
    {
        if (NULL != funcPtrExtraMatchTest)
            if (OK > (status = funcPtrExtraMatchTest(pBucket->pAppData, pTestData, &isMatch)))
                goto exit;
...
extern MSTATUS
HASH_TABLE_traversePtrTable(hashTableOfPtrs *pFreeHashTable,
...
void *
HASH_TABLE_iteratePtrTable(hashTableOfPtrs *pHashTable, void ** pBucketCookie, ubyte4 *index)
{
    hashTablePtrElement*    pBucket;
    hashTablePtrElement*    pNextBucket;
    intBoolean              lastElm = FALSE;

    if ((!index) || (!pBucketCookie))
        goto exit;

    if (NULL != pHashTable)
    {
        for (; *index <= pHashTable->hashTableSizeMask; (*index)++)
        {
            if (NULL != (pBucket = pHashTable->pHashTableArray[*index]))
            {
                while (pBucket)
                {
                    if ((NULL == *pBucketCookie) || (lastElm))
                    {
                        *pBucketCookie = pBucket;
                        return pBucket->pAppData;
                    }

                    pNextBucket = pBucket->pNextElement;
                    if (*pBucketCookie == pBucket)
                    {
                        if (pNextBucket)
                        {
                            *pBucketCookie = pNextBucket;
