...
ASN1_advanceStream(CStream as, ASN1_ITEM* pItem)
{
    ...
    if (filePos + (sbyte4)pItem->length > fileSize)
    {
        return ERR_EOF;
    }
    ...
    *pIsPrimitive = TRUE;
    switch(item->tag)
    {
        ...
        case BITSTRING:
            ...
            if (OK > (status = CS_getc(as, (ubyte *)&item->data.m_unusedBits)))
                goto exit;
...
ASN1_CompareItems( ASN1_ITEM* pItem1, CStream s1, ASN1_ITEM* pItem2, CStream s2)
{
    ...
    item1Data = CS_memaccess( s1, pItem1->dataOffset, (/*FSL*/sbyte4)pItem1->length);
    item2Data = CS_memaccess( s2, pItem2->dataOffset, (/*FSL*/sbyte4)pItem2->length);

    /* MOC_MEMCMP will check for null pointer */
    status = MOC_MEMCMP( item1Data, item2Data, pItem1->length, &cmpRes);
