...
static MSTATUS OCSP_MESSAGE_sign(MOC_RSA_HASH(hwAccelDescr hwAccelCtx) DER_ITEMPTR pSignedHead, ocspContext *pOcspContext, const ubyte* signAlgoOID);
static MSTATUS OCSP_MESSAGE_rsaSignAux(MOC_RSA_HASH(hwAccelDescr hwAccelCtx) RSAKey *pRSAKey, DER_ITEMPTR pToSign, ubyte signAlgo, ubyte **ppSignature, sbyte4 *sigLen);
                                              ...
                                              ubyte **ppSerialNumber, ubyte4 *pLen)
{
    ...
    if (OK > (status = CERT_getCertificateIssuerSerialNumber(pCertRoot, &pIssuer, &pSerialNum)))
        goto exit;

    if (pIssuerDN)
    {
        if (OK > (status = CERT_extractDistinguishedNamesFromName (pIssuer, cs, pIssuerDN)))
            goto exit;
    }
    ...
    if (ppSerialNumber)
    {
        ...
        *pLen = pSerialNum->length;
        ...
        MOC_MEMCPY(pSerialNumber, buf, pSerialNum->length);
                     ...
                     const ubyte* hashAlgo, ubyte* pHashBuf, ubyte4* pHashLen)
{
    ...
    *pHashLen = 0;
    ...
    MOC_MEMCPY(pHashBuf, pHash, digestSize);
    *pHashLen = digestSize;
...
static MSTATUS
OCSP_MESSAGE_rsaSignAux(MOC_RSA_HASH(hwAccelDescr hwAccelCtx) RSAKey *pRSAKey,
                        ...
                        ubyte **ppSignature, sbyte4 *pSigLen)
{
    ...
    if (OK > (status = RSA_getCipherTextLength(pRSAKey, &temp)))
        goto exit;
...
static MSTATUS
OCSP_MESSAGE_sign(MOC_RSA_HASH(hwAccelDescr hwAccelCtx) DER_ITEMPTR pSignedHead,
...

{
    ...
    switch(signKey.type)
    {
        case akt_rsa:
        {
            ...
            if (OK > (status = OCSP_MESSAGE_rsaSignAux(MOC_RSA_HASH(hwAccelCtx) signKey.key.pRSA,
        ...
        case akt_dsa:
        {
            ...
            MOC_MEMCMP(signAlgoOID, dsaWithSHA1_OID, dsaWithSHA1_OID[0]+1, &cmpRes);
            if (0 != cmpRes)
            {
                status = ERR_CERT_AUTH_BAD_SIGN_ALGO;
                goto exit;
...
OCSP_MESSAGE_addServiceLocator(OCSP_singleRequestInfo *pSingleRequestInfo, extensions **ppExt)
{
    ...
    *ppExt = NULL;
    ...
    ASN1_GetChildWithOID(pAIA, cs, id_pe_authorityInfoAcess_OID, &pAIA);
    if (NULL == pAIA)
    {
        /* Not found; Not an Error */
        goto genExt;
                                     ...
                                     OCSP_singleRequest *pSingleRequest)
{
    ...
    if ((NULL == pSingleRequest) || (NULL == pOcspContext) || (NULL == pCertSerialNo))
    {
        status = ERR_NULL_POINTER;
        goto exit;
    }

    if (OK > (status = MOC_MEMSET((ubyte *)pSingleRequest,0x00, sizeof(OCSP_singleRequest))))
        goto exit;
    ...
    MOC_MEMCPY(pSingleRequest->certId.nameHash, pNameHash, pSingleRequest->certId.hashLength);
    ...
    MOC_MEMCPY(pSingleRequest->certId.keyHash, pKeyHash, pSingleRequest->certId.hashLength);
    ...
    MOC_MEMCPY(pOcspContext->ocspProcess.client.issuerNameHash, pNameHash, hashSize);
    MOC_MEMCPY(pOcspContext->ocspProcess.client.issuerPubKeyHash, pKeyHash, hashSize);
    ...
    if (pOcspContext->pOcspSettings->shouldAddServiceLocator)
    {
        ...
        if (pServiceExt)
        {
            ...
            MOC_MEMSET((ubyte *)pExts, 0x00, sizeof(extensions)*(pSingleRequest->extNumber));
            MOC_MEMCPY(pExts, pSingleRequest->singleRequestExtensions,sizeof(extensions)*(pSingleRequest->extNumber-1));
            MOC_MEMCPY((pExts+pSingleRequest->extNumber-1), pServiceExt, sizeof(extensions));
                                   ...
                                   OCSP_singleRequest *pSingleRequest)
{
    ...
    if (OK > (status = MOC_MEMSET((ubyte *)pIssuerInfo, 0x00, sizeof(certDistinguishedName))))
        goto exit;
    ...
    else /* else same Issuer no need to retrieve issuer cert again */
    {
        ...
        MOC_MEMCPY(pSingleRequest->certId.nameHash, pOcspContext->ocspProcess.client.issuerNameHash, pSingleRequest->certId.hashLength);
        ...
        MOC_MEMCPY(pSingleRequest->certId.keyHash, pOcspContext->ocspProcess.client.issuerPubKeyHash, pSingleRequest->certId.hashLength);
    }

    /** RHS values will be uninitialized in case of same issuer **/
    pSingleRequest->certId.serialNumber       = pSerialNumber;
                                     ...
                                     ubyte** ppRetRequest, ubyte4* pRetRequestLen)
{
    ...
    if (pOcspContext->pOcspSettings->shouldSign)
    {
        ...
        if (OK > (status = OCSP_MESSAGE_sign(MOC_RSA_HASH(pOcspContext->hwAccelCtx) pRequest,pOcspContext,
                                    ...
                                    ubyte * pIssuerCert, ubyte4 issuerCertLen)
{
    ...
    if (OK > (status = CERT_checkCertificateIssuer(pCertRoot, csCert, pIssuerRoot, csIssuer)))
        goto exit;
...
OCSP_MESSAGE_getResponderId(ocspContext *pOcspContext, OCSP_responderId **ppResponderId)
{
    ...
    if (1 == pItem->tag)
    {
        ...
        if (OK > (status = CERT_extractDistinguishedNamesFromName(pItem, pOcspContext->ocspProcess.client.cs, pResponderId->value.pName)))
            goto exit;
    ...
    if (2 == pItem->tag)
    {
        ...
        if (20 == pItem->length) /* sha-1 hash; specified in the RFC; hence lenght 20*/
        {
            ...
            MOC_MEMCPY(pResponderId->value.keyHash, responderIdBuf, 20);
            if (responderIdBuf)
            {
...
OCSP_MESSAGE_verifySignature(ocspContext *pOcspContext, ASN1_ITEMPTR pResponseRoot, CStream cs)
{
    ...
    ubyte4            count             = 0;
    OCSP_responderId* pResponderId      = NULL;
    byteBoolean       isTrustedResp     = FALSE;
    MSTATUS           status            = OK;
    ...
    if (OK > (status = GetCertOID(pSeqAlgoId, cs, pkcs1_OID, &oidSuffix, NULL)))
        goto exit;
    ...
    if (OK > (status = CERT_ComputeBufferHash(MOC_HASH(pOcspContext->hwAccelCtx) (ubyte*)buffer,
            ...
            while (1)
            {
                if (pOcspContext->ocspProcess.client.pIssuerRoot)
                {
                    ...
                    if (OK == ASN1_GetNthChild(pIssuerKey, 2, &pIssuerKey))
                    {
                        ...
                        status = CERT_ComputeBufferHash(MOC_HASH(pOcspContext->hwAccelCtx) (ubyte*)keyBuf,
                                                        pIssuerKey->length, pKeyHash,
                                                        &keyHashLen, sha1withRSAEncryption);

                        MOC_MEMCMP(pResponderId->value.keyHash, pKeyHash, keyHashLen, &hashCmpResult);
    ...
    MOC_MEMCPY(&pOcspContext->ocspProcess.client.responderCs, &responderCs, sizeof(CStream));
    ...
    if (OK > (status = CERT_extractRSAKey(MOC_RSA(pOcspContext->hwAccelCtx) pResponderPubKey, responderCs, pResponderKey)))
        goto exit;

    signatureBuf = (ubyte*)CS_memaccess(cs, pSignature->dataOffset, pSignature->length);

    switch(pResponderKey->type)
    {
        case akt_rsa:
        {
            status = CERT_decryptRSASignatureBuffer(MOC_RSA(pOcspContext->hwAccelCtx) pResponderKey->key.pRSA,
    ...
    if (OK > (status = MOC_MEMCMP(pComputedHash, decryptedHash,
...
OCSP_MESSAGE_checkResponderId(ocspContext *pOcspContext)
{
    ...
    certDistinguishedName*  pTRespInfo              = NULL;
    ...
    if (OK > (status = CERT_extractDistinguishedNamesFromName(pResponderCertIssuer, pOcspContext->ocspProcess.client.responderCs, pResponderInfo)))
        goto exit;
    ...
    while (1)
    {
        ...
        if (OK == status)
        {
            ASN1_ITEM *pPublicKeyInfo = NULL;

            /* Verify whether the responder certificate is truly issued by the issuer */
            CRYPTO_initAsymmetricKey(&pubKey);

            if (OK > (status = OCSP_MESSAGE_getCertificateChild(pOcspContext->ocspProcess.client.pIssuerRoot, cert_subjectPublicKeyInfo, &pPublicKeyInfo)))
                goto exit;

            /* verify the type */
            if ((UNIVERSAL != (pPublicKeyInfo->id & CLASS_MASK)) || (SEQUENCE != pPublicKeyInfo->tag))
            {
                status = ERR_CERT_INVALID_STRUCT;
                goto exit;
            }

            if (OK > (status = CERT_extractRSAKey(MOC_RSA(hwAccelCtx) pPublicKeyInfo, pOcspContext->ocspProcess.client.issuerCs, &pubKey)))
                goto exit;

            status = CERT_verifySignature(pOcspContext->ocspProcess.client.pResponderCert,  pOcspContext->ocspProcess.client.responderCs, &pubKey);
...
exit:
    ...
    if (pTRespInfo)
    {
        CA_MGMT_freeCertDistinguishedName(&pTRespInfo);
    }
...
OCSP_MESSAGE_checkNonce(ocspContext *pOcspContext)
{
    ...
    if (pNonce->length == pOcspContext->ocspProcess.client.nonceLen)
    {
        ...
        if (OK > (status = MOC_MEMCMP(pOcspContext->ocspProcess.client.nonce, nonceBuf, pNonce->length, &cmpResult)))
            goto exit;
...
OCSP_MESSAGE_validateCertInfo(ocspContext *pOcspContext,OCSP_certID *pCertId)
{
    ...
    {
        if (OK > (status = MOC_MEMCMP(pCertId->serialNumber,
    ...
    else
    {
        ...
        if (OK > (status = MOC_MEMCMP(pCertId->nameHash,
        ...
        if (OK > (status = MOC_MEMCMP(pCertId->keyHash,
                           ...
                           ubyte* pResponses, ubyte4 responsesLen)
{
    ...
    */
    ...
    switch ((ubyte4)pTemp->data.m_intVal)
    {
        ...
        default:
        {
            pOcspContext->ocspProcess.client.status = ocsp_unknown;
    ...
    */
    ...
    MOC_MEMCPY(pTempRecdData, responseBuf, pResponse->length);
...
OCSP_MESSAGE_getCurrentCertId(ocspContext *pOcspContext, OCSP_certID **ppCertId)
{
    ...
    *ppCertId = NULL;
    ...
    MOC_MEMSET((ubyte *)pTempCertId, 0x00, sizeof(OCSP_certID));
    ...
    MOC_MEMCPY(pTempCertId->nameHash, buf, pItem1->length);
    CS_stopaccess(pOcspContext->ocspProcess.client.cs, buf);
    ...
    MOC_MEMCPY(pTempCertId->keyHash, buf, pItem1->length);
    CS_stopaccess(pOcspContext->ocspProcess.client.cs, buf);
    ...
    MOC_MEMCPY(pTempCertId->serialNumber, buf, pItem1->length);
...
OCSP_MESSAGE_getCurrentCertStatus(ocspContext *pOcspContext, OCSP_certStatus **ppStatus)
{
    ...
    MOC_MEMSET((ubyte*)pTempStatus, 0x00, sizeof(OCSP_certStatus));
...
OCSP_MESSAGE_getCurrentNextUpdate(ocspContext *pOcspContext, TimeDate *pTime, byteBoolean *pIsNextUpdate)
{
    ...
    if (OK > (status = ASN1_GetNthChild(pOcspContext->ocspProcess.client.pSingleResponse, 4, &pNextUpdate)))
    {
        ...
        {
            /* Next Update is not present, NOT ERROR as optional */
            *pIsNextUpdate = FALSE;
            status         = OK;
            goto exit;
        }
