...
AESCMAC_init(MOC_SYM(hwAccelDescr hwAccelCtx) const ubyte *keyMaterial, sbyte4 keyLength, AESCMAC_Ctx *pCtx)
{
    ...
    if (OK != getFIPS_powerupStatus(FIPS_ALGO_AES_CMAC))
        return getFIPS_powerupStatus(FIPS_ALGO_AES_CMAC);
              ...
              AESCMAC_Ctx* pCtx)
{
    ...
    status = AES_OMAC_final(MOC_SYM(hwAccelCtx) &pCtx->aesCtx,
                            &pCtx->omacCtx, cmac);
    ...
    MOC_MEMSET((ubyte *)pCtx, 0x00, sizeof(AESCMAC_Ctx));
...
AES_OMAC_init(AES_OMAC_Ctx* pOMACCtx)
{
    ...
    MOC_MEMSET( pOMACCtx->currBlock, 0, AES_BLOCK_SIZE);
    return OK;
}
