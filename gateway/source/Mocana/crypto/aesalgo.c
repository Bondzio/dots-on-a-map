...
aesKeySetupEnc(ubyte4 rk[/*4*(Nr + 1)*/], const ubyte cipherKey[], sbyte4 keyBits)
{
    ...
    if (OK > MOC_alloc(sizeof(cavmCipherContext), (void **)&ctx))
    {
        printf("aesKeySetupEnc: MOC_alloc() failed\n");
        return 0;
    }

    MOC_MEMSET((ubyte *)ctx, 0x00, sizeof(cavmCipherContext));
    MOC_MEMCPY(&(ctx->key[0]), cipherKey, keyBits);
...
aesKeySetupDec(ubyte4 rk[/*4*(Nr + 1)*/], const ubyte cipherKey[], sbyte4 keyBits)
{
    ...
    if (OK > MOC_alloc(sizeof(cavmCipherContext), (void **)&ctx))
    {
        printf("aesKeySetupDec: MOC_alloc() failed");
        return 0;
    }
    ...
    switch (pAesContext->mode)
    {
        ...
        case MODE_CBC:
        {
...
#endif
            {
                for (i = numBlocks; i > 0; i--)
                {
                    ...
                    MOC_MEMCPY(iv, outBuffer, AES_BLOCK_SIZE);
                    input += AES_BLOCK_SIZE;
                    outBuffer += AES_BLOCK_SIZE;
                }
            }
        ...
        case MODE_CFB128:
        {
            ...
            for (i = numBlocks; i > 0; i--) {
                ...
                MOC_MEMCPY(outBuffer, iv, AES_BLOCK_SIZE);
                outBuffer += AES_BLOCK_SIZE;
                input += AES_BLOCK_SIZE;
            }
        ...
        case MODE_OFB:
        {
            ...
            for (i = numBlocks; i > 0; i--) {
                aesEncrypt(pAesContext->rk, pAesContext->Nr, iv, tmpBlock);
                MOC_MEMCPY(iv, tmpBlock, AES_BLOCK_SIZE);
                for (j = 0; j< AES_BLOCK_SIZE; j++) {
        ...
        case MODE_CBC:
        {
...
#endif
            {
                for (i = numBlocks; i > 0; i--)
                {
                    ...
                    MOC_MEMCPY(iv, input, AES_BLOCK_SIZE);
                    MOC_MEMCPY(outBuffer, block, AES_BLOCK_SIZE);
                    input += AES_BLOCK_SIZE;
                    outBuffer += AES_BLOCK_SIZE;
                }
            }
