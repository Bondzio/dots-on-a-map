...
MSTATUS MD4Update(MOC_HASH(hwAccelDescr hwAccelCtx) MD4_CTX* pContext, const ubyte* input, ubyte4 inputLen)
{
    ...
    if (inputLen >= partLen)
    {
        MOC_MEMCPY(&pContext->buffer[index], input, partLen);
    ...
    MOC_MEMCPY(&pContext->buffer[index], input+i, inputLen-i);
...
MSTATUS MD4Final (MOC_HASH(hwAccelDescr hwAccelCtx) MD4_CTX* pCtx, ubyte digest[MD4_DIGESTSIZE])
{
    ubyte bits[8];
    ubyte4 index, padLen;
