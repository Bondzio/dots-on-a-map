                         ...
                         const ubyte block[16])
{
    ...
    MOC_MEMCPY(x, state, 16);
    MOC_MEMCPY(x+16, block, 16);
    for(i = 0; i < 16; i++)
...
MD2Init(MOC_HASH(hwAccelDescr hwAccelCtx) MD2_CTX *pContext)
{
    ...
    pContext->count = 0;
    if(OK >(status = MOC_MEMSET(pContext->state, 0x00, sizeof(pContext->state))))
        goto exit;

    status = MOC_MEMSET(pContext->checksum, 0x00, sizeof(pContext->checksum));
          ...
          const ubyte *pInput, ubyte4 inputLen)
{
    ...
    if (inputLen >= partLen)
    {
        if (OK > (status = MOC_MEMCPY(&pContext->buffer[index], pInput, partLen)))
            goto exit;
    ...
    status = MOC_MEMCPY(&pContext->buffer[index], &pInput[i], inputLen-i);
...
MD2Final(MOC_HASH(hwAccelDescr hwAccelCtx) MD2_CTX *pContext, ubyte digest[16])
{
    ...
    if (OK > (status = MOC_MEMSET(pad, (ubyte)padLen, padLen)))
        goto exit;
    ...
    if (OK > (status = MOC_MEMCPY(digest, pContext->state, 16)))
        goto exit;

    /* clear memory */
    status = MOC_MEMSET((ubyte *)pContext, 0, sizeof(*pContext));
