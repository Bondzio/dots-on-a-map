...
SHA1_freeDigest(MOC_HASH(hwAccelDescr hwAccelCtx) BulkCtx *pp_context)
{
   ...
   if (OK != getFIPS_powerupStatus(FIPS_ALGO_SHA1))
    ...
    MOC_MEMSET(*pp_context,0x00,sizeof(shaDescr));
...
SHA1_updateDigest(MOC_HASH(hwAccelDescr hwAccelCtx) shaDescr *p_shaContext, const ubyte *pData, ubyte4 dataLen)
{
   ...
   if (OK != getFIPS_powerupStatus(FIPS_ALGO_SHA1))
    ...
    if (p_shaContext->hashBufferIndex > 0)
    {
        ...
        MOC_MEMCPY( p_shaContext->hashBuffer + p_shaContext->hashBufferIndex, pData, numToCopy);
        pData += numToCopy;
        dataLen -= numToCopy;
    ...
    if (dataLen > 0)
    {
        MOC_MEMCPY(p_shaContext->hashBuffer + p_shaContext->hashBufferIndex, pData, dataLen);
...
#endif
    /* Zeroize the sensitive information before deleting the memory */
    MOC_MEMSET((unsigned char *)&shaContext,0x00,sizeof(shaDescr));
...
#endif /* ((!(defined(__SHA1_HARDWARE_HASH__))) || (!(defined(__DISABLE_MOCANA_RNG__)))) */
